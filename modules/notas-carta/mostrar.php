...
// Obtiene las ventas
$ventas = $db->select('i.*, a.almacen, a.principal, e.nombres, e.paterno, e.materno')->from('inv_egresos i')->join('inv_almacenes a', 'i.almacen_id = a.id_almacen', 'left')->join('sys_empleados e', 'i.empleado_id = e.id_empleado', 'left')->where('i.empleado_id', $_user['persona_id'])->where('i.tipo', 'Venta')->where('i.codigo_control', '')->where('i.provisionado', 'S')->where('i.fecha_egreso >= ', $fecha_inicial)->where('i.fecha_egreso <= ', $fecha_final)->order_by('i.fecha_egreso desc, i.hora_egreso desc')->fetch();
...
<div class="panel-heading" data-formato="<?= strtoupper($formato_textual); ?>" data-mascara="<?= $formato_numeral; ?>" data-gestion="<?= date_decode($gestion_base, $_institution['formato']); ?>">
	<h3 class="panel-title">
		...
		<strong>Notas de remisión personales</strong>
	</h3>
</div>
<div class="panel-body">
	...
	<div class="row">
		...
		<div class="col-xs-12 col-sm-4 text-right">
			...
			<a href="?/notas/crear" class="btn btn-primary"><span class="glyphicon glyphicon-plus"></span><span class="hidden-xs"> Vender</span></a>
			<?php } ?>
		</div>
	</div>
	<hr>
		...
		<tbody>
			...
			<tr>
				...
				<td class="text-nowrap">
					<?php if ($permiso_ver) { ?>
					<a href="?/notas/ver/<?= $venta['id_egreso']; ?>" data-toggle="tooltip" data-title="Ver detalle de nota de remisión"><i class="glyphicon glyphicon-list-alt"></i></a>
					<?php } ?>
