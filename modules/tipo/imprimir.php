...
if ($id_categoria == 0) {
	...
	$categorias = $db->select('z.*')
	->from('inv_categorias z')
	->order_by('z.id_categoria')
	->fetch();
} else {
...
if ($id_categoria == 0) {
	...
	$pdf->Cell(0, 10, 'LISTA DE CATEGORíAS', 0, true, 'C', false, '', 0, false, 'T', 'M');
	...
	foreach ($categorias as $nro => $categoria) {
		$body .= '<tr>';
		$body .= '<td>' . ($nro + 1) . '</td>';
		$body .= '<td>' . escape($categoria['categoria']) . '</td>';
		$body .= '<td>' . escape($categoria['descripcion']) . '</td>';
		$body .= '</tr>';
	}
	
	$body = ($body == '') ? '<tr><td colspan="3" align="center">No existen categorías registradas en la base de datos</td></tr>' : $body;
...
EOD;
	...
	$nombre = 'categorias_' . date('Y-m-d_H-i-s') . '.pdf';
} else {
	...
	// Titulo del documento
	$pdf->Cell(0, 10, 'DETALLE DE CATEGORíA', 0, true, 'C', false, '', 0, false, 'T', 'M');
	
	// Establece la fuente del titulo
	$pdf->SetFont('helvetica', 'I', 10);
	
	// Subtitulo del documento
	$pdf->Cell(0, 5, 'ID: ' . $id_categoria, 0, true, 'C', false, '', 0, false, 'T', 'M');
	...
	$valor_categoria = escape($categoria['categoria']);
	$valor_descripcion = escape($categoria['descripcion']);
			...
			<td width="80%" style="border: 1px solid #444;">
				<table cellpadding="5">
					<tr>
						...
						<td width="60%">$valor_categoria</td>
					</tr>
					<tr>
