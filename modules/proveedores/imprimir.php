...
$pdf->Cell(0, 10, 'LISTA DE PROVEEDORES', 0, true, 'C', false, '', 0, false, 'T', 'M');
...
// Estructura la tabla
$body = '';
foreach ($proveedores as $nro => $proveedor) {
	$body .= '<tr>';
	$body .= '<td>' . ($nro + 1) . '</td>';
	$body .= '<td>' . escape($proveedor['nombre_proveedor']) . '</td>';
	$body .= '<td>' . escape($proveedor['nro_visitas']) . '</td>';
	$body .= '<td>' . escape($proveedor['total_compras']) . '</td>';
	$body .= '</tr>';
}

$body = ($body == '') ? '<tr><td colspan="5" align="center">No existen proveedores registrados en la base de datos</td></tr>' : $body;
...
$nombre = 'proveedores_' . date('Y-m-d_H-i-s') . '.pdf';
