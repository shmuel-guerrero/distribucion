...
//        $mayores = cantidad_unidad($db,$detalle['id_producto'],$id_caja);
        $id_productoc = $detalle['id_producto'];
        $mayores = $db->query("SELECT * FROM inv_asignaciones WHERE visible = 's' and producto_id = '$id_productoc' AND cantidad_unidad < '$cantidades' ORDER BY cantidad_unidad ")->fetch_first();

        if(isset($mayores) && $mayores['cantidad_unidad'] <= $cantidades){
            $unidad_mayor = $mayores['unidad_id'];
            $mayor = $mayores['cantidad_unidad'];
        }else{
...
//         $total = $total + $detalle['m_total'];
        ...
        $body .= '<td class="left-right bot" align="left">' . escape($detalle['codigo']) . '</td>';
    ...
    foreach ($detalles2 as $nro => $detalle2) {
        //$total = $total + $detalle['prec'];
        $importe = 0;
        $sugerido = escape($detalle2['precio_sugerido']);
        //var_dump($detalle);exit();
        // $cantidad = escape($detalle['cantidad']);

        // $precio = escape($detalle['precio']);
        $descuento = escape($detalle2['descuento']);
        
        //$total = $total + $importe;
        $unid = explode('|', $detalle2['cantidades']);
        $precios = explode('|', $detalle2['precios']);
        $mayor = 0;
        $unidad_mayor = 0;
        $cantidades = 0;
        if(count($unid)>1){
            ...
            foreach ($unid as $nro2 => $uni) {
                ...
                //$importe = ($cantid / cantidad_unidad($db,$detalle['id_producto'],$unidad)) * $precios[$nro2];
                       //Validacion
                if(cantidad_unidad($db,$detalle2['id_producto'],$unidad))
                    $importe = ($cantid / cantidad_unidad($db,$detalle['id_producto'],$unidad)) * $precios[$nro2];
            ...
            $cantid = $parte[0];
            $cantidades = $cantidades + $cantid;
            $importe = ($cantid/cantidad_unidad($db,$detalle2['id_producto'],$unidad))* $precios[0];
            $importe_t = $importe;
            $total = $total + $importe;
        }
        // buscar id caja;
        $mayores = cantidad_unidad($db,$detalle2['id_producto'],$id_caja);

        if(isset($mayores) && $mayores <= $cantidades){
            $unidad_mayor = $id_caja;
            $mayor = $mayores;
        }else{
            $unidad_mayor = $detalle2['unidad_producto'];
            $mayor = 1;
        }
        ...
        if($cantidades % $mayor == 0){
            ...
            $otra_unidad = '<br>'.nombre_unidad($db,$detalle2['unidad_producto']);
            $otra_cantid = '<br>'.($cantidades-($unidades_t*$mayor));
        }
//        $unidades4 = substr($unidades4,0,-4);
//         $total = $total + $detalle['m_total'];
        ...
        $body .= '<td class="left-right bot" align="right">' . $unidades_t .''.$otra_cantid. '</td>';
        $body .= '<td class="left-right bot">' . nombre_unidad($db,$unidad_mayor).'('.$mayor.' U.)'.''.$otra_unidad. '</td>';
        $body .= '<td class="left-right bot" align="left">' . escape($detalle2['nombre_factura']) . '</td>';
        $body .= '<td class="left-right bot" align="left">' . escape($detalle2['codigo']) . '</td>';
        //$body .= '<td class="left-right bot" align="right">' . $detalle2['descripcion']. '</td>';
        $body .= '<td class="left-right bot" align="right">' . $detalle2['categoria']. '</td>';
        $body .= '<td class="left-right bot" align="right">' . number_format($importe_t, 2, '.', '') . '</td>';
        $body .= '</tr>';
    }
...
// Formateamos la tabla
    ...
    <table cellpadding="3" class="bor">
		<tr>
            ...
            <th width="17%" class="all" align="left">UNIDAD</th>
