...
// Obtiene las ventas
$proformas = $db->select('i.*, e.nombres, e.paterno, e.materno, f.motivo, e.fecha')
    ->from('inv_egresos i')
    ->join('gps_rutas a', 'i.ruta_id = a.id_ruta', 'left')
    ->join('sys_empleados e', 'a.empleado_id = e.id_empleado', 'left')
    ->join('gps_noventa_motivos f', 'i.motivo_id = f.id_motivo', 'left')
    ->where('i.fecha_egreso = e.fecha')
    ->where('i.estadoe',2)
    ->where('i.ruta_id',$empleado)
    ->where('i.grupo','')->order_by('i.fecha_egreso desc, i.hora_egreso desc')->fetch();
// echo $db->last_query();
        ...
        <?php if ($permiso_cambiar || $permiso_imprimir) { ?>
            <div class="row">
                ...
                <div class="col-xs-12 col-sm-4 text-right">
                    <a href="?/distribuidor/listar" class="btn btn-success">
                    ...
                    <table id="table" class="table table-bordered table-condensed table-striped table-hover table-xs">
                        ...
                        <?php foreach ($proformas as $nro => $proforma) {
                            ...
                            <tr>
                                ...
                                <td class="text-nowrap"><?= escape($proforma['observacion']); ?></td>
                                <?php if ($permiso_facturar || $permiso_ver || $permiso_eliminar || $permiso_imprimir) { ?>
                ...
                <div class="col-sm-6">
                    <div class="row">
                        ...
                        <div class="col-sm-6 text-right">
                            <a href="?/distribuidor/ver2/<?= $empleado ?>" class="btn btn-success" >
    ...
    $(function () {
        ...
        $.validate({
            ...
            onSuccess: function () {
                ...
                window.location = '?/distribuidor/visitas2/' + empleado + inicial_fecha + final_fecha;
            }
        });
            ...
            var control = L.Routing.control({
                ...
                plan: L.Routing.plan(waypoints, {
                    createMarker: function(i, wp) {
                        if(estados[i]==1){
                            return L.marker( wp.latLng, {icon: redIcon }).bindPopup('cliente: '+nombres[i]).openPopup();
                        }else{
                            return L.marker( wp.latLng, {icon: greenIcon }).bindPopup('cliente: '+nombres[i]).openPopup();
                        }
                    }
                }),
