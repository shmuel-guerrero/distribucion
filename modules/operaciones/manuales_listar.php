					...
					<tr>
						...
						<td class="text-nowrap">
							...
							<?php if ($permiso_anular && date('Y-m-d') <= $masUnMesdate) { ?>
								<a href="?/operaciones/activar_manuales/<?= $venta['id_egreso']; ?>" data-toggle="tooltip" style="margin-right: 5px" data-title="<?= $estadoAnulado ? 'Anular' : 'Activar' ?> (puede <?= $estadoAnulado ? 'anular' : 'activar' ?> venta hasta el <?= $masUnMesdate; ?>)."><i class="text-<?= $estadoAnulado ? 'info' : 'warning' ?> glyphicon glyphicon-<?= $estadoAnulado ? 'remove-circle' : 'ok-circle' ?>"></i></a>
							<?php } ?>
