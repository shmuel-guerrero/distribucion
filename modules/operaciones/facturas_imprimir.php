...
if ($id_egreso == 0) {
	// Obtiene las egresos
	$egresos = $db->select('p.*, a.almacen, a.principal, e.nombres, e.paterno, e.materno, cl.credito, cl.dias')
					->from('inv_egresos p')
					->join('inv_almacenes a', 'p.almacen_id = a.id_almacen', 'left')
					->join('sys_empleados e', 'p.empleado_id = e.id_empleado', 'left')
					->join('inv_clientes as cl', 'p.cliente_id = cl.id_cliente', 'left')
	...
	$permiso_ver = in_array('facturas_imprimir', $permisos);
	
	// Obtiene la egreso
	$egreso = $db->select('p.*, a.almacen, a.principal, e.nombres, e.paterno, e.materno, cl.credito, cl.dias')
				  ->from('inv_egresos p')
				  ->join('inv_almacenes a', 'p.almacen_id = a.id_almacen', 'left')
				  ->join('sys_empleados e', 'p.empleado_id = e.id_empleado', 'left')
				  ->join('inv_clientes as cl', 'p.cliente_id = cl.id_cliente', 'left')
	...
	$detalles = $db->select('d.*, d.unidad_id as unidad_otra, p.codigo, p.nombre, p.nombre_factura, if(asi.cantidad_unidad is null, 1, asi.cantidad_unidad) as cantidad_unidad, u.sigla')
...
if ($id_egreso == 0) {
	...
	// descuento
	$valor_descuento = escape($egreso['descuento_bs']);
	$valor_total_descuento = ($egreso['monto_total_descuento'] == 0)?$egreso['monto_total']:$egreso['monto_total_descuento'];
	$valor_descuento_porc = isset($egreso['descuento_porcentaje']) ? '('.escape($egreso['descuento_porcentaje']).'%)' : '';
	...
	$valor_total_desc = number_format($valor_total_descuento , 2, '.', '');
	...
	$monto_textual = explode('.', $valor_total_desc);
	...
	<h1 align="center">FACTURA <br><b style="font-size: 12px; color:red;">COPIA</b></h1>
	<table cellpadding="1">
		<tr>
	...
	<table cellpadding="5">
		...
		$body
		<tr>
			<th class="all" align="left" colspan="5">TOTAL:</th>
			<th class="all" align="right">$valor_total</th>
		</tr>
		<tr>
			<th class="all" align="left" colspan="5">DESCUENTO $valor_descuento_porc:</th>
			<th class="all" align="right">$valor_descuento</th>
		</tr>
		<tr>
			<th class="all" align="left" colspan="5">SON: $monto_escrito</th>
			<th class="all" align="right">$valor_total_descuento</th>
		</tr>
	</table>
	<br><br>
	<table cellpadding="1">
		<tr>
			...
			<td width="20%" class="none">$valor_codigo</td>
			<td width="20%" class="none" rowspan="2" align="right">
			
			</td>
			<td width="40%" class="none" rowspan="2" align="right">
	...
	<div width="100%" align="left">$credito</div>
EOD;
