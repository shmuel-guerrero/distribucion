...
$permiso_imprimir = in_array('proformas_imprimir', $permisos) || true;
...
<div class="panel-body">
	...
	<div class="row">
		...
		<div class="col-xs-12 col-sm-9 text-right">
			...
			<!-- <a href="?/operaciones/notas_imprimir/<?= $proforma['id_proforma']; ?>" class="btn btn-default" target="_blank"><i class="glyphicon glyphicon-file"></i><span class="hidden-xs"> Exportar</span></a> -->
			<a href="?/proformas/imprimir/<?= $proforma['id_proforma']; ?>" class="btn btn-info" target="_blank"><i class="glyphicon glyphicon-print"></i><span class="hidden-xs"> Reimprimir</span></a>
			<?php } ?>
			<?php if (true) { ?>
				<button type="button" class="btn btn-success" data-reimprimir="true"><i class="glyphicon glyphicon-retweet"></i><span class="hidden-xs"> Convertir</span></button>
			<?php } ?>
			<?php // if ($permiso_facturar) { if(!$proforma['nro_autorizacion']){?>
			<!-- <a href="?/operaciones/proforma_ver/<?= $proforma['id_proforma']; ?>" class="btn btn-success"><i class="glyphicon glyphicon-qrcode"></i><span class="hidden-xs hidden-sm"> Facturar</span></a> -->
			<?php // } }?>
			<?php if ($permiso_eliminar) { ?>
			<a href="?/operaciones/proformas_eliminar/<?= $proforma['id_proforma']; ?>" class="btn btn-danger" data-eliminar="true"><span class="glyphicon glyphicon-trash"></span><span class="hidden-xs hidden-sm"> Eliminar</span></a>
			<?php } ?>
			<?php if ($permiso_editar) { ?>
			<button type="button" class="btn btn-warning" data-editar="true"><i class="glyphicon glyphicon-edit"></i><span class="hidden-xs hidden-sm"> Editar datos del cliente</span></button>
			<?php } ?>
			<?php if ($permiso_listar) { ?>
			<a href="?/operaciones/proformas_listar" class="btn btn-primary"><i class="glyphicon glyphicon-list-alt"></i><span class="hidden-xs hidden-sm"> Listado de prorfomas</span></a>
			<?php } ?>
		</div>
	</div>
	<hr>
						...
						<table id="table" class="table table-bordered table-condensed table-restructured table-striped table-hover">
							<thead>
								<tr class="active">
									...
									<th class="text-nowrap text-center">Subtotal <?= escape($moneda); ?></th>
								</tr>
							</thead>
							<tbody>
								<?php $total = 0; ?>
							...
							<tfoot>
								...
								<tr class="active">
									<th class="text-nowrap text-right" colspan="5">Descuento <?= $proforma['descuento_porcentaje'] != 0 ? ' ('.$proforma['descuento_porcentaje'].'%) ': '' ?><?= escape($moneda); ?></th>
									<th class="text-nowrap text-right"><?= number_format($proforma['descuento_bs'], 2, '.', ''); ?></th>
								</tr>							
								<tr class="active">
...
<div id="modal_cliente" class="modal fade">
	<div class="modal-dialog">
		<form method="post" action="?/proformas/proformas_editar" id="form_cliente" class="modal-content">
