...
if (is_ajax() && is_post()) {
	...
	if (isset($_POST['id_egreso'])) {
		...
		// Obtiene el egreso
		$id_egreso = trim($_POST['id_proforma']);
		...
		$egreso = $db->select('i.*, a.almacen, a.principal, e.nombres, e.paterno, e.materno')
					...
					->join('sys_empleados e', 'i.empleado_id = e.id_empleado', 'left')->where('id_egreso', $id_egreso)->fetch_first();
		...
		if ($egreso) {
			...
			$monto_textual = explode('.', $egreso['monto_total']);
