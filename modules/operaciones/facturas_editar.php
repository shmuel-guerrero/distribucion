...
if (is_post()) {	
	...
	if (isset($_POST['id_egreso']) && isset($_POST['nit_ci']) && isset($_POST['nombre_cliente'])) {
		...
		if ($venta) {
			...
			$verifica = backup_registros($db, 'inv_egresos_detalles', 'egreso_id', $id_egreso, '', '', $_user['persona_id'], 'NO', $verifica_id, "Editado");

			// Obtiene la dosificacion
			$dosificacion = $db->from('inv_dosificaciones')->where('id_dosificacion', $venta['dosificacion_id'])->where('activo','S')->fetch_first();
			...
			if ($dosificacion) {
				...
				redirect('?/operaciones/facturas_ver/' . $id_egreso);
			} else {
				// Error 404
