...
// Obtiene el producto
$egreso = $db->select('*')->from('inv_egresos')->where('id_egreso',$id_egreso)->fetch_first();
$detalles = $db->query("SELECT a.*, b.*,e.categoria, a.unidad_id AS unidad_det, d.unidad
	...
	WHERE a.egreso_id = '$id_egreso' GROUP BY a.id_detalle")->fetch();
...
$permiso_listar = in_array('preventas_listar', $permisos);
...
<?php require_once show_template('header-advanced'); ?>
    ...
    <?php if ($almacen) { ?>
        <div class="col-md-6">
            <div class="panel panel-default">
                ...
                <h2 class="lead text-info">Devolución <?= escape($almacen['almacen']); ?></h2>
					<hr>
                        ...
                        <div class="table-responsive margin-none">
                            <table id="ventas" class="table table-bordered table-condensed table-striped table-hover table-xs margin-none">
                                ...
                                <tr class="active">
                                    ...
                                    <th class="text-nowrap">Cantidad devuelta</th>
            ...
            <div class="panel panel-default">
                ...
                <div class="panel-body">
                    <?php if ($permiso_listar) { ?>
                        <div class="row">
                            <div class="col-xs-12 text-right">
                                <a href="?/operaciones/preventas_listar" class="btn btn-success"><i class="glyphicon glyphicon-list-alt"></i><span> Listado de preventas</span></a>                                
                        ...
                        <hr>
                    <?php } ?>
                    <?php if ($detalles) {  // echo json_encode($detalles);  ?>
                        <table id="productos" class="table table-bordered table-condensed table-striped table-hover table-xs">
                            ...
                            <?php foreach ($detalles as $nro => $detalle) {
                                ...
                                // $otro_precio = $db->select('*')->from('inv_asignaciones a')->join('inv_unidades b','a.unidad_id=b.id_unidad')->where('a.producto_id',$detalle['id_producto'])->fetch();
                                ...
                                <tr>
                                    ...
                                    <td class="text-nowrap text-right" data-stock="<?= $detalle['id_producto']; ?>"><?= escape($cantidad); ?></td>
                                    <td class="text-nowrap text-right" data-valor="<?= $detalle['id_producto']; ?>">
                                        ...
                                        *<?= escape('(1)'.$detalle['unidad'].': '); ?><b><?= escape($lote['costo']); ?></b>
                                        <?php // foreach($otro_precio as $otro){ $aux = $aux.'*'.$otro['cantidad_unidad']?>
                                            <!--<br/>*<?php // escape('('.$otro['cantidad_unidad'].')'.$otro['unidad'].': '); ?><b><?php // escape($otro['cantidad_unidad']*$lote['costo']); ?></b>-->
                                        <?php // } ?>
                                    </td>
                                    ...
                                    <td class="hidden" data-cant="<?= $detalle['id_producto']; ?>"><?= escape($cantidad); ?></td>
                                    <td class="hidden" data-stock2="<?= $detalle['id_producto']; ?>"><?= escape($cantidad); ?></td>
                                </tr>
    ...
    function adicionar_producto(id_producto) {
        ...
        if ($producto.size()) {
            ...
            '<td><input type="text" value="1" name="cantidades[]" class="form-control input-xs text-right" maxlength="7" autocomplete="off" data-cantidad="" data-validation="required number" data-validation-allowing="range[1;' + stock + ']" data-validation-error-msg="Debe ingresar un número positivo entre 1 y ' + stock + '"  onkeyup="calcular_importe(' + id_producto + ')"></td>';
            if(porciones.length>2){
                ...
                '<td><input type="text" value="' + aparte[1] + '" name="precios[]" class="form-control input-xs text-right" autocomplete="off" data-precio="' + aparte[1] + '"  readonly  data-validation="required number" data-validation-allowing="range[0.1;10000000.00],float" data-validation-error-msg="Debe ingresar un número decimal positivo mayor que 0 y menor que 10000000" onkeyup="calcular_importe(' + id_producto + ')"></td>';
            }
            else{
                ...
                '<td><input type="text" value="' + parte[1] + '" name="precios[]" class="form-control input-xs text-right" autocomplete="off" data-precio="' + parte[1] + '" data-validation="required number" data-validation-allowing="range[0.1;10000000.00],float" data-validation-error-msg="Debe ingresar un número decimal positivo mayor que 0 y menor que 10000000" onkeyup="calcular_importe(' + id_producto + ')"></td>';
            }
