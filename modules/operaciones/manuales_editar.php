...
//EGRESO
$Egreso = $db->query("SELECT e.id_egreso,e.monto_total,e.nro_registros,e.tipo,e.fecha_egreso,e.hora_egreso,e.descripcion,e.nro_factura,e.nro_autorizacion,e.codigo_control,e.nombre_cliente,e.nit_ci,e.estadoe,
    ...
    <div class='col-md-6'>
        <div class='panel panel-default'>
            <div class='panel-heading'>
                <h3 class='panel-title'>
                    <span class='glyphicon glyphicon-option-vertical'></span>
                    <strong>Editar venta manual</strong>
                </h3>
            </div>
            <div class='panel-body'>
				...
				<?php if (isset($_SESSION[temporary])) { ?>
					...
					<?php unset($_SESSION[temporary]); ?>
				<?php } ?>
                <form id='formulario' class='form-horizontal' method='post' action='?/operaciones/manuales_actualizar'>
					...
					<div class="form-group">
						...
						<div class="col-sm-8">
							<input type="text" value="<?= $Egreso['nro_factura']?>" name="nro_factura" id="nro_factura" class="form-control text-uppercase" autocomplete="off" data-validation="required number">
						</div>
					</div>
					<div class="form-group">
						<label for="nro_autorizacion" class="col-sm-4 control-label">Número de autorización:</label>
						<div class="col-sm-8">
							<input type="text" value="<?= $Egreso['nro_autorizacion']?>" name="nro_autorizacion" id="nro_autorizacion" class="form-control text-uppercase" autocomplete="off" data-validation="required number">
						</div>
					</div>
                            ...
                            <tbody>
								<?php foreach($DetalleEgreso as $key => $detalle){?>
                                    ...
                                    <tr class="active" data-producto="<?= $detalle['producto_id'] ?>">
                                        ...
                                        <td>
                                        	<input type="text" value="<?= $detalle['cantidad']/cantidad_unidad($db,$detalle['producto_id'],$detalle['unidad_det']) ?>" name="cantidades[]" class="form-control input-xs text-right" maxlength="7" autocomplete="off" data-cantidad="" data-validation="required number" data-validation-allowing="range[1;<?= ($resta_stock + $detalle['cantidad']); ?>]" data-validation-error-msg="Debe ingresar un número positivo entre 1 y <?= ($resta_stock + $detalle['cantidad']); ?>" onkeyup="calcular_importe(<?= $detalle['producto_id'] ?>)" >
                                        </td>
                                        <?php if(false){
//                                        if($detalle['prec']){?>
                                            <td>
		...
		<div class='row'>
			<div class='col-md-12'> <!-- col-sm-10 col-sm-offset-1 -->
				<div class='panel panel-default'>
					<div class='panel-heading'>
						<h3 class='panel-title'><i class='glyphicon glyphicon-log-out'></i> Información de la venta manual</h3>
					</div>
					<div class='panel-body'>
...
<script>
		...
		if ($producto.size()) {
			...
			if (porciones.length > 2) {
				...
				plantilla = plantilla + '</select></td>' +
					'<td><input type="text" value="' + aparte[1] + '" name="precios[]" class="form-control input-xs text-right" autocomplete="off" data-precio="' + aparte[1] + '" data-validation="required number" data-validation-allowing="range[0.1;10000000.00],float" data-validation-error-msg="Debe ingresar un número decimal positivo mayor que 0 y menor que 10000000" onkeyup="calcular_importe(' + id_producto + ')"></td>';
			} else {
				...
				plantilla = plantilla + '<td><input type="text" value="' + parte[0] + '" data-xyyz="' + stock + '" name="unidad[]" class="form-control input-xs text-right" autocomplete="off" data-unidad="' + parte[0] + '" readonly data-validation-error-msg="Debe ser un número decimal positivo"></td>' +
					'<td data-xyyz="' + stock + '" ><input type="text" value="' + parte[1] + '" name="precios[]" class="form-control input-xs text-right" autocomplete="off"  data-precio="' + parte[1] + '" data-cant2="1" data-validation="required number" data-validation-allowing="range[0.1;10000000.00],float" data-validation-error-msg="Debe ingresar un número decimal positivo mayor que 0 y menor que 10000000" onkeyup="calcular_importe(' + id_producto + ')"></td>';
			}
	...
	function guardar_nota() {
		...
		$.ajax({
			type: 'post',
			dataType: 'json',
			url: '?/operaciones/manuales_actualizar',
			data: data
		}).done(function(venta) {
