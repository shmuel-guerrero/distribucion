...
$permiso_facturar = false ; // in_array('preventas_facturar', $permisos); // volver a esconder
		...
		<tbody>
			...
			<tr>
				...
				<td class="text-nowrap">
                    ...
                    <?php if ($permiso_facturar) { if(!$proforma['nro_autorizacion'] && $proforma['estadoe'] == 3 && $fecha_registro_mas_mes > date('Y-m-d')) {?>
                        <a href="?/operaciones/preventa_ver/<?= $proforma['id_egreso']; ?>" data-toggle="tooltip"  style="margin-right: 5px"  data-title="Convertir en facturar"><i class="glyphicon glyphicon-qrcode"></i></a>
                    <?php } else if($proforma['estadoe'] == 3 && $proforma['nro_autorizacion']){?>
                        <a  data-toggle="tooltip" data-title="Ya se facturo"><i class="glyphicon glyphicon-qrcode text-primary"></i></a>
                    <?php }} ?>
                    <?php if ($permiso_ver) { ?>
					...
					<?php if ($proforma['estadoe'] == 3 && $proforma['evento'] == 'Ninguno' && validar_atributo($db, $_plansistema['plan'], module, file, 'devoluciones')) { ?> <!-- cambiar 2 a 3 -->
