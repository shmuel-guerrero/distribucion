...
$movimientos = $db->query("
                            SELECT c.*, d.motivo, e.*, b.materno as maternox, b.paterno as paternox, b.nombres as nombresx 
                            FROM sys_users a
                            ...
                            WHERE  a.rol_id != 4 
                                    AND c.cliente_id = '$id_cliente' 
                                    AND c.distribuidor_fecha >= '$fecha_inicial' 
                                    AND c.distribuidor_id <= '$fecha_final'
                        ")->fetch();
            ...
            <div class="col-sm-6">
                <div class="well">
                    ...
                    <dl class="margin-none">
                        <dt>Nombre:</dt>
                        <dd><?= escape($cliente['cliente']); ?></dd>
                        <dt>Nombre factura:</dt>
                        <dd><?= escape($cliente['nombre_factura']); ?></dd>
                    </dl>
                </div>
            </div>
            <div class="col-sm-6">
            ...
            <div class="table-responsive">
                <table id="table" class="table table-bordered table-condensed table-restructured table-striped table-hover">
                    ...
                    <?php foreach($movimientos as $nro => $movimiento){
                        $productos = $db->select('*, a.unidad_id as unidad_venta')
                                        ->from('tmp_egresos_detalles a')
                                        ->join('inv_productos b','a.producto_id=b.id_producto')
                                        ->where(array('a.egreso_id'=>$movimiento['id_egreso'],'tmp_egreso_id'=>$movimiento['id_tmp_egreso']))
                                        ->fetch();
                    ...
                    <tr>
                        ...
                        <td class="text-nowrap success text-primary"><?= (escape($movimiento['paternox']." ".$movimiento['maternox']." ".$movimiento['nombresx'])); ?></td>
