...
<div class="panel-body">
	<div class="row">
		...
		<div class="col-xs-12 col-sm-3 text-right">
			<select id="roles" class="form-control form-inline">
				<option value="?/tools/asignar_permiso">Listado</option>
				<?php foreach ($roles as $elemento) { ?>
					<?php if ($elemento['id_rol'] == $id_rol) { ?>
					<option value="?/tools/asignar_permiso_lista/<?= $elemento['id_rol']; ?>" selected><?= escape($elemento['rol']); ?></option>
					<?php } else { ?>
					<option value="?/tools/asignar_permiso_lista/<?= $elemento['id_rol']; ?>"><?= escape($elemento['rol']); ?></option>
					<?php } ?>
				<?php } ?>
			</select>
		</div>
	...
	<hr>
	<?php if ($menus) { ?>
	<form method="post" action="?/tools/asignar_permiso_guardar" class="form-horizontal">
