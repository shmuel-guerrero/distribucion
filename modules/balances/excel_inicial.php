...
$objPHPExcel->getProperties()
    ...
    ->setTitle("Balance inicial")
    ->setSubject("Balance inicial")
    ->setDescription("Balance inicial")
    ->setKeywords("Excel Office 2007 php")
    ->setCategory("Balance");

$objPHPExcel->getActiveSheet()->mergeCells('A2:D2')->setCellValue('A2', 'Balance inicial');
...
foreach ($aux2 as $au2) {
    $cue = $db->query("SELECT c.n_plan,c.plan_cuenta,SUM(a.debe),SUM(a.haber) FROM con_asiento a,con_plan c WHERE a.comprobante=" . $a . " AND a.cuenta=c.n_plan AND c.estado=1 AND c.n_plan like '" . $au2['id_cuenta'] . "%' GROUP BY c.n_plan ORDER BY c.nodo ASC,LENGTH(c.n_plan) ASC,c.n_plan ASC")->fetch();
    if ($au2['id_cuenta'] == 1) {
    ...
    foreach ($aux1 as $aux) {
        $act = $db->query("SELECT c.n_plan,c.plan_cuenta,SUM(a.debe),SUM(a.haber) FROM con_asiento a,con_plan c WHERE a.comprobante=" . $a . " AND a.cuenta=c.n_plan AND c.estado=1 AND c.n_plan like '" . $aux['n_plan'] . "%' GROUP BY c.n_plan ORDER BY c.nodo ASC,LENGTH(c.n_plan) ASC,c.n_plan ASC")->fetch();
        if ($aux['nodo'] == 1) {
...
header('Content-Disposition: attachment;filename="inicial'.date('Y-m-d').'.xlsx"');
