...
$pdf->Cell(0, 15, ' Estado de Resultados ',0, true, 'C', false, '', 0, false, 'T', 'M');
...
$aux2=$db->select('*')->from('con_cuenta')->where('id_cuenta>',3)->where('id_cuenta<',6)->fetch();
foreach($aux2 as $au2){
    ...
    if($au2['id_cuenta']==4){$totc = sumar_vec($cue,'SUM(a.debe)')-sumar_vec($cue,'SUM(a.haber)');$t1=$t1+$totc;}else{$totc = sumar_vec($cue,'SUM(a.haber)')-sumar_vec($cue,'SUM(a.debe)');$t2=$t2+$totc;}
    $pdf->SetFont('Roboto', 'B', 10);
...
// Multicell test
    ...
    $pdf->MultiCell(80, 20, '', 0, 'R', 1, 1, '', '', true, 0, false, true, 20, 'M');
    ...
    foreach($aux1 as $aux){
        ...
        foreach($act as $ac){
            if($aux['nodo']==4){if($ac['SUM(a.debe)']>$ac['SUM(a.haber)']){$acd=$ac['SUM(a.debe)']-$ac['SUM(a.haber)'];$g1=$g1+$acd;}else{$acd=$ac['SUM(a.haber)']-$ac['SUM(a.debe)'];$v1=$v1+$acd;}}
            else{if($ac['SUM(a.debe)']<$ac['SUM(a.haber)']){$acd=$ac['SUM(a.haber)']-$ac['SUM(a.debe)'];$v1=$v1+$acd;}else{$acd=$ac['SUM(a.debe)']-$ac['SUM(a.haber)'];$g1=$g1+$acd;}};
            if($acd!=0){
    ...
    $pdf->MultiCell(305, 20, 'UTILIDAD IMPONIBLE', 0, 'L', 1, 0, '', '', true, 0, false, true, 20, 'M');
    $pdf->MultiCell(80, 20, convercion($v1-$g1,1,$db), 0, 'R', 1, 1, '', '', true, 0, false, true, 20, 'M');
    $t2=$t2+$t1-$t2;

$pdf->SetFont('Roboto', 'B', 10);
$pdf->MultiCell(80, 20, '', 0, 'J', 1, 0, '', '', true, 0, false, true, 20, 'M');
$pdf->MultiCell(305, 20, 'IUE', 0, 'L', 1, 0, '', '', true, 0, false, true, 20, 'M');
$pdf->MultiCell(80, 20, convercion(($v1-$g1)*0.25,1,$db), 0, 'R', 1, 1, '', '', true, 0, false, true, 20, 'M');

$pdf->MultiCell(80, 20, '', 0, 'J', 1, 0, '', '', true, 0, false, true, 20, 'M');
$pdf->MultiCell(305, 20, 'UTILIDAD NETA', 0, 'L', 1, 0, '', '', true, 0, false, true, 20, 'M');
$pdf->MultiCell(80, 20, convercion(($v1 - $g1)-(($v1-$g1)*0.25),1,$db), 0, 'R', 1, 1, '', '', true, 0, false, true, 20, 'M');
