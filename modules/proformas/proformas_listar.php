...
// Obtiene las ventas
$proformas = $db->select('i.*, a.almacen, a.principal, e.nombres, e.paterno, e.materno, e.cargo')
->from('inv_proformas i')->join('inv_almacenes a', 'i.almacen_id = a.id_almacen', 'left')->join('sys_empleados e', 'i.empleado_id = e.id_empleado', 'left')
->where('i.fecha_proforma >= ', $fecha_inicial)
->where('i.fecha_proforma <= ', $fecha_final)
// ->where('i.estadoe>',1)
->where('e.id_empleado',$_user['persona_id'])->order_by('i.fecha_proforma desc, i.hora_proforma desc')->fetch();
	...
	<div class="row">
		...
		<div class="col-xs-12 col-sm-4 text-right">
            ...
            <?php if ($permiso_imprimir) { ?>
                <a href="?/proformas/proformas_imprimir" class="btn btn-primary" target="_blank" data-imprimir="true"><span class="glyphicon glyphicon-file"></span><span class="hidden-xs"> Imprimir</span></a>
            <?php } ?>
		</div>
	</div>
	<hr>
			...
			<tr>
				...
				<td class="text-nowrap">
                    <?php if ($permiso_facturar) { if(!$proforma['nro_autorizacion']){?>
                        <a href="?/operaciones/preventa_ver/<?= $proforma['id_proforma']; ?>" data-toggle="tooltip" data-title="Convertir en facturar"><i class="glyphicon glyphicon-qrcode"></i></a>
                    <?php } else{?>
                        <a  data-toggle="tooltip" data-title="Ya se facturo"><i class="glyphicon glyphicon-qrcode"></i></a>
                    <?php }} ?>
                    <?php if ($permiso_ver) { ?>
                        <a href="?/operaciones/proformas_ver/<?= $proforma['id_proforma']; ?>" data-toggle="tooltip" data-title="Ver detalle de la preventa"><i class="glyphicon glyphicon-list-alt"></i></a>
					<?php } ?>
					<?php if ($permiso_eliminar) { ?>
                        <a href="?/operaciones/proformas_eliminar/<?= $proforma['id_proforma']; ?>" data-toggle="tooltip" data-title="Eliminar preventa" data-eliminar="true"><span class="glyphicon glyphicon-trash"></span></a>
					<?php } ?>
				</td>
				<?php } ?>
                <!--<th class="text-nowrap">-->
...
$(function () {
	...
	$.validate({
		...
		onSuccess: function () {
			...
			window.location = '?/proformas/proformas_listar' + inicial_fecha + final_fecha;
		}
	});
