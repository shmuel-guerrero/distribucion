...
$detalles = $db->select('d.*, p.codigo, p.nombre, p.descripcion, m.marca')
...
$monto_total_descuento = escape($proforma['monto_total_descuento']);
$descuento_bs = escape($proforma['descuento_bs']);
$descuento_porcentaje = escape($proforma['descuento_porcentaje']);
$total = 0;
...
$pdf->Cell($width_table * 0.5, $height_cell, (($_institution['nombre']) ? $_institution['nombre'] : 'Checkcode'), 0, 1, 'L', 0, '', 1, true, 'T', 'M');
$pdf->Cell($width_table * 0.5, $height_cell, (($_institution['lema']) ? $_institution['lema'] : 'Software and technology service'), 0, 1, 'L', 0, '', 1, true, 'T', 'M');
$pdf->Cell($width_table * 0.5, $height_cell, (($_institution['direccion']) ? $_institution['direccion'] : 'First in research, development and innovation'), 0, 1, 'L', 0, '', 1, true, 'T', 'M');
...
foreach ($detalles as $nro => $detalle) {
	...
	$body .= '<td width="52%" align="justify"><b>' . escape($detalle['nombre']) . ' <small class="text-success"> ' . (($detalle['marca']) ? escape($detalle['marca']) : '') . '</small>' . '</b>'./*<br><em>' . str_replace("\n", '<br>', $detalle['descripcion']) . '</em>*/'</td>';
...
if($proforma['descuento_bs'] != 0){
	...
	$pdf->Cell($width_page * 0.24, $height_cell * 1.5, number_format($descuento_bs, 2, '.', ','), 0, 1, 'R', 0, '', 1, true, 'T', 'M');

	$pdf->Cell($width_page * 0.76, $height_cell * 1.5, 'IMPORTE TOTAL ' . $moneda . ' / ' . $monto_literal . ' ' . $monto_decimal . '/100', 0, 0, 'L', 0, '', 1, true, 'T', 'M');
	$pdf->Cell($width_page * 0.24, $height_cell * 1.5, number_format($monto_total_descuento, 2, '.', ','), 0, 1, 'R', 0, '', 1, true, 'T', 'M');
