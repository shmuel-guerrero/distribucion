...
//validar que usuario no tiene creado una meta o tiene un rango de fecha asignado vigente
$Consulta = $db->query("SELECT id_empleado,nombres,paterno,materno, m.fecha_registro, m.fecha_inicio, m.fecha_fin FROM sys_empleados e 
                            LEFT JOIN (SELECT MAX(fecha_registro)AS fecha_registro, empleado_id, fecha_inicio, fecha_fin FROM  inv_meta GROUP BY empleado_id) m ON e.id_empleado = m.empleado_id
                            LEFT JOIN sys_users u ON u.persona_id = e.id_empleado
                            WHERE  u.rol_id = 3 AND ( CURDATE() NOT BETWEEN  m.fecha_inicio AND  m.fecha_fin 
...
<div class='panel-heading' data-formato='<?= strtoupper($formato_textual); ?>' data-mascara='<?= $formato_numeral; ?>' data-gestion='<?= date_decode($gestion_base, $_institution['formato']); ?>'>
    <h3 class='panel-title'>
        ...
        <strong>Crear Metas de Vendedor</strong>
    </h3>
</div>
<div class='panel-body'>
    <div class='row'>
        ...
        <div class='col-xs-12 col-sm-4 text-right'>
            <a href='?/metas/listar' class='btn btn-primary'><i class='glyphicon glyphicon-list-alt'></i><span> Listado</span></a>
        </div>
    </div>
    <div class='row'>
        <div class='col-sm-6 col-sm-offset-3'>
            <form method='post' id='meta_form' action='?/metas/guardar' class='form-horizontal' enctype='multipart/form-data'>
