<?php
    ...
    $Consulta=$db->query("SELECT*FROM inv_meta WHERE id_meta='{$id_meta}'")->fetch_first();
    $Conseguido=$db->query("SELECT IFNULL(SUM(monto_total),0)AS total FROM inv_egresos WHERE empleado_id='{$Consulta['empleado_id']}' AND fecha_egreso BETWEEN '{$Consulta['fecha_inicio']}' AND '{$Consulta['fecha_fin']}'")->fetch_first();
