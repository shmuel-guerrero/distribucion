...
if ($id_moneda == 0) {
	...
	$pdf->Cell(0, 10, 'LISTA DE MONEDAS', 0, true, 'C', false, '', 0, false, 'T', 'M');
	...
	// Estructura la tabla
	$body = '';
	foreach ($monedas as $nro => $moneda) {
		$body .= '<tr>';
		$body .= '<td>' . ($nro + 1) . '</td>';
		$body .= '<td>' . escape($moneda['moneda']) . '</td>';
		$body .= '<td>' . escape($moneda['sigla']) . '</td>';
		$body .= '<td>' . ((escape($moneda['oficial']) == 'S') ? 'Si' : 'No') . '</td>';
		$body .= '</tr>';
	}
	
	$body = ($body == '') ? '<tr><td colspan="4" align="center">No existen monedas registradas en la base de datos</td></tr>' : $body;
	...
	<table cellpadding="5">
		<tr>
			...
			<th width="24%">Sigla</th>
			<th width="30%">Oficial</th>
		</tr>
		$body
	</table>
EOD;
	...
	$nombre = 'monedas_' . date('Y-m-d_H-i-s') . '.pdf';
} else {
	...
	// Titulo del documento
	$pdf->Cell(0, 10, 'DETALLE DE MONEDA', 0, true, 'C', false, '', 0, false, 'T', 'M');
	
	// Establece la fuente del titulo
	$pdf->SetFont('helvetica', 'I', 10);
	
	// Subtitulo del documento
	$pdf->Cell(0, 5, 'ID: ' . $id_moneda, 0, true, 'C', false, '', 0, false, 'T', 'M');
	...
	$valor_moneda = escape($moneda['moneda']);
	$valor_sigla = escape($moneda['sigla']);
	$valor_oficial = (escape($moneda['oficial']) == 'S') ? 'Si' : 'No';
		...
		<tr>
			...
			<td width="80%" style="border: 1px solid #444;">
				<table cellpadding="5">
					...
					<tr>
						<th width="40%">Oficial:</th>
						<td width="60%">$valor_oficial</td>
					</tr>
				</table>
			</td>
		</tr>
...
EOD;
	...
	// Genera el nombre del archivo
	$nombre = 'moneda_' . $id_moneda . '_' . date('Y-m-d_H-i-s') . '.pdf';
}
