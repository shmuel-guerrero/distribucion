...
// Obtiene el producto
$producto_e = $db->select('z.*, a.unidad, a.sigla, b.categoria')->from('inv_productos z')->join('inv_unidades a', 'z.unidad_id = a.id_unidad', 'left')->join('inv_categorias b', 'z.categoria_id = b.id_categoria', 'left')->where('z.id_producto', $id_producto)->fetch_first();
$promociones = $db->select('a.precio, a.cantidad, a.producto_id, b.codigo, b.nombre, b.nombre_factura, c.unidad')->from('inv_promociones a')->join('inv_productos b','a.producto_id = b.id_producto')->join('inv_unidades c','b.unidad_id = c.id_unidad')->where('a.id_promocion',$id_producto)->fetch();

$stock = $db->from('inv_ingresos_detalles')->where('producto_id', $producto_e['id_producto'])->fetch_first();
$para_al = $db->select('i.almacen_id')->from('inv_ingresos_detalles id')->join('inv_ingresos i', 'id.ingreso_id = i.id_ingreso', 'left')->where('id.producto_id', $producto_e['id_producto'])->fetch_first();

// Obtiene el almacen principal
$almacen = $db->from('inv_almacenes')->fetch_first();
//echo json_encode($almacen);
//exit();
$id_almacen = ($para_al) ? $para_al['almacen_id'] : 0;
...
<?php require_once show_template('header-configured'); ?>
            ...
            <div class="col-md-6">
                <div class="panel panel-default">
                    ...
                    <div class="panel-body">
                        ...
                        <?php } ?>
                        <form method="post" action="?/promociones/guardar_promocion2" class="form-horizontal">
                            <div class="form-group">
                                <label for="codigo" class="col-md-3 control-label">Código:</label>
                                <div class="col-md-9">
                                    <input type="hidden" value="<?= $producto_e['id_producto']?>" name="id_producto" data-validation="required">
                                    <input type="text" value="<?= $producto_e['codigo']?>" name="codigo" id="codigo" class="form-control" data-validation-allowing="-/.#º() " data-validation-length="max50" data-validation-url="?/promociones/validar">
                                </div>
                            </div>
                            <div class="form-group">
                                <label for="nombre" class="col-md-3 control-label">Nombre del promoción:</label>
                                <div class="col-md-9">
                                    <input type="text" value="<?= $producto_e['nombre'] ?>" name="nombre" id="nombre" class="form-control" data-validation="required letternumber length" data-validation-allowing='-+/.,:;#&º"() ' data-validation-length="max100">
                                </div>
                            </div>
                            <div class="form-group">
                                ...
                                <div class="col-md-9">
                                    <select name="categoria_id" id="categoria_id" class="form-control" data-validation="required">
                                        ...
                                        <?php foreach ($categorias as $elemento) {
                                            if($producto_e['categoria_id'] == $elemento['id_categoria']){?>
                            ...
                            <div class="form-group">
                                ...
                                <div class="col-md-9">
                                    <input type="text" value="<?= $stock['cantidad']; ?>" name="stock" id="stock" data-limite="" class="form-control" data-validation="required number">
                                </div>
                            </div>
                            <div class="form-group">
                            ...
                            <div class="table-responsive margin-none">
                                <table id="ventas" class="table table-bordered table-condensed table-striped table-hover table-xs margin-none">
                                    ...
                                    <?php foreach($promociones as $key => $promocion){ ?>
                                        <tr class="active" data-producto="<?= $promocion['producto_id'] ?>">
                                            <td class="text-nowrap"><?= $key +1 ?></td>
                                            <td class="text-nowrap"><input type="text" value="<?= $promocion['producto_id'] ?>" name="productos[]" id="producto" class="translate" tabindex="-1" data-validation="required number" data-validation-error-msg="Debe ser número"><?= $promocion['codigo'] ?></td>
                                            <td><input type="text" value="<?= $promocion['nombre']?>" name="nombres[]" class="translate" tabindex="-1" data-validation="required"><?= $promocion['nombre'] ?></td>
                                            <td><input type="text" value="<?= $promocion['cantidad'] ?>" name="cantidades[]" class="form-control input-xs text-right" maxlength="7" autocomplete="off" data-cantidad="" data-validation="required number" onkeyup="calcular_importe(<?= $promocion['producto_id'] ?>), calcular_stock(<?= $promocion['producto_id'] ?>)"></td>
                                            <td><input type="text" value="<?= $promocion['unidad'] ?>" name="unidad[]" class="form-control input-xs text-right" autocomplete="off" data-unidad="<?= $promocion['unidad_id'] ?>" readonly data-validation-error-msg="Debe ser un número decimal positivo"></td>
                                            <td><input type="text" value="<?= $promocion['precio'] ?>" name="precios[]" class="form-control input-xs text-right" autocomplete="off" data-precio="<?= $promocion['precio'] ?>"  data-validation-error-msg="Debe ser un número decimal positivo" onkeyup="calcular_importe(<?= $promocion['producto_id'] ?>), calcular_stock(<?= $promocion['producto_id'] ?>)"></td>
                                            <td class="hidden"><input type="text" value="0" name="descuentos[]" class="form-control input-xs text-right" maxlength="2" autocomplete="off" data-descuento="0" data-validation="required number" data-validation-allowing="range[0;50]" data-validation-error-msg="Debe ser un número positivo entre 0 y 50" onkeyup="descontar_precio(<?= $promocion['producto_id'] ?>)"></td>
                                            <td class="text-nowrap text-right" data-importe="<?= $promocion['cantidad']*$promocion['precio'] ?>"><?= $promocion['cantidad']*$promocion['precio'] ?></td>
                                            <td class="text-nowrap text-center">
                                                <button type="button" class="btn btn-xs btn-danger" data-toggle="tooltip" data-title="Eliminar producto" tabindex="-1" onclick="eliminar_producto(<?= $promocion['producto_id'] ?>)"><span class="glyphicon glyphicon-remove"></span></button>
                                                </td>
                                            </tr>
                                    <?php } ?>
                                    </tbody>
    ...
    $(function () {
        ...
        $('[data-actualizar]').on('click', function () {
            ...
            $.ajax({
                ...
                if (producto) {
                    var precio = parseFloat(producto.precio).toFixed(3);
    ...
    function descontar_precio(id_producto) {
        ...
        $producto.find('[data-precio]').val(precio.toFixed(3));
