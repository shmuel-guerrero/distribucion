...
if ($id_almacen != 0) {
	...
	/*$productos = $db->query("select p.id_producto, p.descripcion, p.imagen, 
	p.codigo, p.nombre, p.nombre_factura, p.cantidad_minima, p.precio_actual, p.unidad_id, 
	ifnull(e.cantidad_ingresos, 0) as cantidad_ingresos, ifnull(s.cantidad_egresos, 0) as cantidad_egresos,
	 IFNULL(e.costo, 0) AS costo_ingresos, u.unidad, u.sigla, c.categoria
	from (select prod.*,asig.precio_actual,asig.unidad_id from inv_productos prod left join inv_asignaciones asig on asig.producto_id=prod.id_producto) as p
	left join (select d.producto_id, d.costo, sum(d.cantidad) as cantidad_ingresos 
	from inv_ingresos_detalles d left join inv_ingresos i on i.id_ingreso = d.ingreso_id 
	where i.almacen_id = $id_almacen  group by d.producto_id ) as e on e.producto_id = p.id_producto 
	left join (select d.producto_id, sum(d.cantidad) as cantidad_egresos from inv_egresos_detalles d 
	left join inv_egresos e on e.id_egreso = d.egreso_id where e.almacen_id = $id_almacen 
	group by d.producto_id ) as s on s.producto_id = p.id_producto 
	left join inv_unidades u on u.id_unidad = p.unidad_id left join inv_categorias c on 
	c.id_categoria = p.categoria_id where p.grupo = '' and p.promocion = '' ")->fetch();*/
	...
	group by d.producto_id ) as s on s.producto_id = p.id_producto 
	left join inv_unidades u on u.id_unidad = p.unidad_id left join inv_categorias c on 
	c.id_categoria = p.categoria_id where p.grupo = '' and p.promocion = '' and p.regalo=1")->fetch();
	...
	/*$detalle2 = $db->query("select p.id_producto, p.descripcion, p.imagen, p.codigo, p.nombre, 
	p.nombre_factura, p.cantidad_minima, a.precio_actual, 
	...
	left join inv_promociones_monto m on m.id_promocion = r.promocion_monto_id
	...
	where m.id_promocion=$id_promocion")->fetch();*/


	$detalle2 = $db->query("select p.id_producto ,p.nombre, p.codigo from inv_productos p left join inv_requisitos_promo r on r.productos_id = p.id_producto where r.promocion_monto_id=$id_promocion")->fetch();

	
	//var_dump($detalle2);die();
...
<div class="col-sm-12 text-left">
	...
	<i class="glyphicon glyphicon-list"></i><span class="hidden-xs"> Listar</span>
	</a><br><br>
			...
			<div class="panel-body">
				...
				<form method="post" action="?/promociones/guardar_promocion_x_fecha" class="form-horizontal">
					<div class="form-group">
						...
						<div class="col-md-9">
							<input type="hidden" value="<?=$promo['nombre'];?>" name="nombre" data-validation="required">
							<input type="hidden" value="<?=$promo['id_promocion'];?>" name="id_promocion" id="id_promocion" class="form-control" data-validation-allowing="-/.#º() " data-validation-length="max50" data-validation-url="?/productos/validar">
					...
					<div class="form-group">
						...
						<div class="col-md-9">
							<input type="text" value="<?=$promo['nombre'];?>" name="nombre" id="nombre" class="form-control" data-validation="required letternumber length" data-validation-allowing='-+/.,:;#&º"() ' data-validation-length="max100" readonly>
						</div>
					</div>
					<div class="form-group">
						<label for="stock" class="col-md-3 control-label">Monto:</label>
						<div class="col-md-9">
							<input type="text" value="<?=$promo['monto_promo'];?>" name="min_promo" id="min_promo" class="form-control" data-validation="required" readonly>
						</div>
					</div>
					<div class="form-group">
						<label for="stock" class="col-md-3 control-label">Fecha Inicio:</label>
						<div class="col-md-9">
							<input type="date" value="<?=$promo['fecha_ini'];?>" name="fecha_ini" id="fecha_ini" class="form-control" data-validation="required date" readonly>
						</div>
					</div>
					<div class="form-group">
						<label for="stock" class="col-md-3 control-label">Fecha Fin:</label>
						<div class="col-md-9">
							<input type="date" value="<?=$promo['fecha_fin'];?>" name="fecha_fin" id="fecha_fin" class="form-control" data-validation="required date date_end" readonly>
						</div>
					</div>
					...
					<div class="form-group">
						...
						<div class="col-md-9">
							<?php if($promo['tipo'] == 2) { ?>
								<input type="text" value="Descuento en Bolivianos"  class="form-control" readonly id="tipo">								
								<?php } else { ?>
					...
					<div class="form-group">
						<label for="almacen" class="col-md-3 control-label">Grupos:</label>
						<div class="col-md-9">
							<?php
								$grupos = $db->select('distinct c.nombre_grupo')
								->from('inv_participantes_promos p')
								->join('inv_clientes_grupos c','p.cliente_grupo_id=c.id_cliente_grupo','left')
								->fetch();							
								$dato = '';
								foreach($grupos as $grupo){
									$dato .= $grupo['nombre_grupo'].', ';
								}
							?>
							<input type="text" value="<?= $dato;?>"  class="form-control" readonly>								
						</div>
					</div>

					<div class="form-group">
						<label for="almacen" class="col-md-3 control-label">Clientes:</label>
						<div class="col-md-9">
							<?php
								$participantes = $db->select('distinct c.cliente,c.nombre_factura')
								->from('inv_participantes_promos p')
								->join('inv_clientes c','p.cliente_id=c.id_cliente','left')
								->fetch();							
								$dato = '';
								foreach($participantes as $participante){
									$dato .= $participante['cliente'].' '.$participante['nombre_factura'].', ';
								}
							?>
							<input type="text" value="<?= $dato;?>"  class="form-control" readonly>								
						</div>
					...
					<div id="monto" style="display:none">
						<div class="form-group">
							<label for="almacen" class="col-md-3 control-label">Monto:</label>
							<div class="col-md-9">
					...
					<!--<div id="rango" style="display:none">
						<div class="form-group">
							<label for="almacen" class="col-md-3 control-label">Porcentaje:</label>
							<div class="col-md-9">-->
								<!--<input type="text" value="" id="descuento_promo" name="descuento_promo" class="form-control text-left" autocomplete="off" data-cuentas="" data-validation="required number" data-validation-allowing="range[1;360],int" data-validation-error-msg="Debe ser número entero positivo">-->
								<!--<input type="text" value="" name="descuento_promo" id="descuento_promo" class="form-control" data-validation="required number">
							</div>
						</div>
					</div>	-->
					...
					<div id="item">
						<div class="table-responsive margin-none">
							<table id="ventas" class="table table-bordered table-condensed table-striped table-hover table-xs margin-none">
								...
								<!--<tbody>
									<tr class="active" data-producto="3">
										<td class="text-nowrap">1</td>
										<td class="text-nowrap"><input type="text" value="3" name="productos[]" class="translate" tabindex="-1" data-validation="required number" data-validation-error-msg="Debe ser número">125</td>
										<td><input type="text" value="product2o" name="nombres[]" class="translate" tabindex="-1" data-validation="required">product2o<input type="hidden" id="id_asign" name="id_asign" value="undefined">  </td>
										<td><input type="text" value="1" name="cantidades[]" class="form-control input-xs text-right" maxlength="7" autocomplete="off" data-cantidad="" data-validation="required number" data-validation-allowing="range[1;100]" data-validation-error-msg="Debe ser un número positivo entre 1 y 100" onkeyup="calcular_importe(3)"></td>
										<td><input type="text" value="" name="unidad[]" class="form-control input-xs text-right" autocomplete="off" data-unidad="" readonly="" data-validation-error-msg="Debe ser un número decimal positivo"></td><td><input type="text" value=" 44.00" name="precios[]" class="form-control input-xs text-right" autocomplete="off" data-precio=" 44.00" data-validation-error-msg="Debe ser un número decimal positivo" onkeyup="calcular_importe(3)"></td>
										<td class="hidden"><input type="text" value="0" name="descuentos[]" class="form-control input-xs text-right" maxlength="2" autocomplete="off" data-descuento="0" data-validation="required number" data-validation-allowing="range[0;50]" data-validation-error-msg="Debe ser un número positivo entre 0 y 50" onkeyup="descontar_precio(3)"></td>
										<td class="text-nowrap text-right" data-importe="">44.00</td><td class="text-nowrap text-center"><button type="button" class="btn btn-xs btn-danger" data-toggle="tooltip" data-title="Eliminar producto" tabindex="-1" onclick="eliminar_producto(3)"><span class="glyphicon glyphicon-remove"></span></button></td>
									</tr>
								</tbody>-->
								<tbody>
								<?php  foreach($detalle2 as $deta){ ?>
									<tr class="active" data-producto="<?= $deta['id_producto'] ?>">
										<td class="text-nowrap">0</td>
										<td class="text-nowrap"><input type="text" value="<?= $deta['id_producto'] ?>" name="productos[]" class="translate" tabindex="-1" ><?= $deta['codigo'] ?></td>
										<td><input type="text" value="<?= $deta['nombre'] ?>" name="nombres[]" class="translate" tabindex="-1" data-validation="required"><?= $deta['nombre'] ?></td>
										<!--<td><input type="text" value="<?= $deta['cantidad_egresos'] ?>" name="cantidades[]" class="form-control input-xs text-right" maxlength="7" ></td>


										<td><input type="text" value="<?//= $deta['unidad'] ?>" name="unidades[]" class="form-control input-xs text-right" ></td>

										<td><input type="text" value="<?//= $deta['precio_actual'] ?>"  name="precios[]" class="form-control input-xs text-right" maxlength="2" autocomplete="off" data-precio="<?//= $deta['precio_actual'] ?>"></td>
										...
										<td class="text-nowrap text-center">
											<button type="button" class="btn btn-xs btn-danger" data-toggle="tooltip" data-title="Eliminar producto" tabindex="-1" onclick="eliminar_producto(<?//= $deta['id_producto']?>)"><span class="glyphicon glyphicon-remove"></span></button>
										</td>
									</tr>-->
								<?php } ?>
								</tbody>
...
$(function () {
	...
	if($('#tipo').val()=='Descuento en Bolivianos')
	{
		$('#busqueda').hide();
		$('#ventas').hide();
	} else {
...
function adicionar_producto(id_producto) {
	...
	if ($producto.size()) {
		...
		plantilla = '<tr class="active" data-producto="' + id_producto + '">' +
						...
						'<td><input type="text" value="' + nombre + '" name="nombres[]" class="translate" tabindex="-1" data-validation="required">' + nombre + '<input type="hidden" id="id_asign" name="id_asign" value="'+ asign +'" />  </td>'/* +
