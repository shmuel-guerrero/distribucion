...
// Obtiene el producto
$producto_e = $db->select('z.*, a.unidad, a.sigla, b.categoria')->from('inv_productos z')->join('inv_asignaciones asi','asi.producto_id = z.id_producto and asi.estado = 1 and asi.tipo_entrada = "principal"')->join('inv_unidades a', 'asi.unidad_id = a.id_unidad', 'left')->join('inv_categorias b', 'z.categoria_id = b.id_categoria', 'left')->where('z.id_producto', $id_producto)->fetch_first();
$promociones = $db->select('a.precio, a.cantidad, a.producto_id, b.codigo, b.nombre, b.nombre_factura, c.unidad')->from('inv_promociones a')->join('inv_productos b','a.producto_id = b.id_producto')->join('inv_asignaciones asi','asi.producto_id = b.id_producto and asi.estado = 1 and asi.tipo_entrada = "principal"')->join('inv_unidades c','asi.unidad_id = c.id_unidad')->where('a.id_promocion',$id_producto)->fetch();
...
if ($id_almacen != 0) {
    ...
    $productos = $db->query("select p.id_producto, p.descripcion, p.imagen, p.codigo, p.nombre, p.nombre_factura, p.cantidad_minima, asi.precio_actual, asi.unidad_id, ifnull(e.cantidad_ingresos, 0) as cantidad_ingresos, ifnull(s.cantidad_egresos, 0) as cantidad_egresos, IFNULL(e.costo, 0) AS costo_ingresos, u.unidad, u.sigla, c.categoria 
        ...
        left join inv_asignaciones asi on asi.producto_id = p.id_producto and asi.estado = 1 and asi.tipo_entrada = 'principal'
        left join inv_unidades u on u.id_unidad = asi.unidad_id 
        left join inv_categorias c on c.id_categoria = p.categoria_id where p.grupo = ''")->fetch();
} else {
    $productos = null;
}
                        ...
                        <form method="post" action="?/promociones/guardar_promocion" class="form-horizontal">
                            <div class="form-group">
                                ...
                                <div class="col-md-9">
                                    ...
                                    <input type="text" value="<?= $producto_e['codigo']?>" name="codigo" id="codigo" class="form-control" data-validation-allowing="-/.#º() " data-validation-length="max50" data-validation-url="?/productos/validar">
                                </div>
                            </div>
                            <div class="form-group">
                                ...
                                <table id="ventas" class="table table-bordered table-condensed table-striped table-hover table-xs margin-none">
                                    ...
                                    <?php foreach($promociones as $key => $promocion){ ?>
                                        <tr class="active" data-producto="<?= $promocion['producto_id'] ?>">
                                            ...
                                            <td><input type="text" value="<?= $promocion['cantidad'] ?>" name="cantidades[]" class="form-control input-xs text-right" maxlength="7" autocomplete="off" data-cantidad="" data-validation="required number" onkeyup="calcular_importe(<?= $promocion['producto_id'] ?>)"></td>
                                            ...
                                            <td><input type="text" value="<?= $promocion['precio'] ?>" name="precios[]" class="form-control input-xs text-right" autocomplete="off" data-precio="<?= $promocion['precio'] ?>"  data-validation-error-msg="Debe ser un número decimal positivo" onkeyup="calcular_importe(<?= $promocion['producto_id'] ?>)"></td>
