<?php
// Obtiene el id_promocion post
$id_promocion_post = (sizeof($params) > 0) ? $params[0] : 0;

// Obtiene el usuario
//$promocion = $db->select('*')->from('inv_promociones')->where('id_promocion', $id_promocion_post)->fetch_first();
$promocion = $db->select('*')->from('inv_promociones_monto')->where('id_promocion', $id_promocion_post)->fetch_first();
		...
		<div class="panel panel-default">
			<div class="panel-heading">
				...
				<h3 class="panel-title">
					...
					<strong>Editar datos de la Promoción</strong>
				</h3>
			</div>
			<div class="panel-body">
                ...
                <form method="post" action="?/promociones/guardar_promocion_x_fecha" class="form-horizontal">
					...
					<div class="form-group">
						...
						<div class="col-md-9">							
                            <input type="hidden" value="<?= $promocion['id_promocion']; ?>" name="id_promocion" data-validation="required">
						    <input type="text" value="<?= $promocion['nombre']; ?>" name="nombre" id="nombre" class="form-control" autocomplete="off" data-validation="required letternumber length" data-validation-allowing=" " data-validation-length="max100">
						</div>
					</div>
					<div class="form-group">
						<label for="stock" class="col-md-3 control-label">Monto:</label>
						<div class="col-md-9">
                        <input type="text" value="<?= $promocion['min_promo']; ?>" name="min_promo" id="min_promo" class="form-control" autocomplete="off" data-validation-allowing=" " data-validation-length="max100" data-validation-optional="true">
						</div>
					</div>
                    <div class="form-group">
                        <label for="fecha_ini" class="col-md-3 control-label">Fecha Incio:</label>
                        <div class="col-md-9">
                            <input type="date" value="<?= date_decode($promocion['fecha_ini'], $_institution['formato']); ?>" name="fecha_ini" id="fecha_ini" class="form-control" autocomplete="off" data-validation="birthdate" data-validation-format="<?= $formato_textual; ?>" data-validation-optional="true">
                        </div>
                    </div>					
					<div class="form-group">
                        <label for="fecha_ini" class="col-md-3 control-label">Fecha Fin:</label>
                        <div class="col-md-9">
                            <input type="date" value="<?= date_decode($promocion['fecha_fin'], $_institution['formato']); ?>" name="fecha_fin" id="fecha_fin" class="form-control" autocomplete="off" data-validation="birthdate" data-validation-format="<?= $formato_textual; ?>" data-validation-optional="true">
                        </div>
                    </div>
					<div class="form-group">
						<label for="descripcion" class="col-md-3 control-label">Descripción:</label>
						<div class="col-md-9">
							<textarea value="" name="descripcion" id="descripcion" class="form-control" rows="3" data-validation="letternumber" data-validation-allowing='-+/.,:;@#&"()_\n ' data-validation-optional="true"><?= $promocion['descripcion']; ?></textarea>
						</div>
					</div>
                    
                    <div class="form-group">
                        <label for="monto_desc" class="col-md-3 control-label">Tipo:</label>
                        <div class="col-md-9">
                            <select name="tipo" id="tipo" class="form-control" data-validation="required number" onchange="set_tipo_promo()">
                                <option value="2" <?php if($promocion['tipo']=="2"){ ?> selected <?php } ?> >Descuento en Bolivianos</option>
                                <option value="4" <?php if($promocion['tipo']=="4"){ ?> selected <?php } ?> >Item</option><!--manda value 4 siempre-->
                            </select>
                        </div>
                    </div>
					<div id="monto_desc" style="display:none">
						<div class="form-group">
							<label for="monto_promo" class="col-md-3 control-label">Descuento en Bs:</label>
							<div class="col-md-9">
                                ...
                                <input type="text" value="<?= $promocion['monto_promo']; ?>" name="monto_promo" id="monto_promo" class="form-control" autocomplete="off" data-validation-allowing=" " data-validation-length="max100" data-validation-optional="true">
							</div>
						</div>
					</div>
                    <?php // set_tipo_promo($promocion['tipo']);?>
							...
							<table id="ventas" class="table table-bordered table-condensed table-striped table-hover table-xs margin-none">
								<thead>
									<tr class="active">
										...
										<th class="text-nowrap" width="30%">Precio</th>
