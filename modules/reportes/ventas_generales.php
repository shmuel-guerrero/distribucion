...
$ventas = $db->query("SELECT i.*,a.almacen,a.principal,e.nombres,e.paterno,e.materno 
				...
				WHERE i.tipo='Venta' AND i.fecha_egreso>='{$fecha_inicial}' AND i.fecha_egreso<='{$fecha_final}' AND i.anulado != 3
...
<div class="panel-heading" data-formato="<?= strtoupper($formato_textual); ?>" data-mascara="<?= $formato_numeral; ?>" data-gestion="<?= date_decode($gestion_base, $_institution['formato']); ?>">
	<h3 class="panel-title">
		...
		<strong>Reporte de ventas generales</strong>
	</h3>
</div>
<div class="panel-body">
	...
	<?php } ?>
	<?php if ($ventas) { ?>
	<table id="table" class="table table-bordered table-condensed table-striped table-hover table-xs" style="width:100%;">
		<thead>
			<tr class="active">
...
$(function () {
	...
	$.validate({
		...
		onSuccess: function () {
			...
			window.location = '?/reportes/ventas_generales' + inicial_fecha + final_fecha;
		}
	});
		...
		var table = $('#table').on('draw.dt', function () { // search.dt order.dt page.dt length.dt
    		...
    		$('#total').text(suma.toFixed(2));
				...
				values: {
					...
					order: [[0, 'asc']],
					ajax: {
						url: '?/reportes/ventas_generales2/<?=$fecha_inicial?>/<?=$fecha_final?>',
						type: 'post',
						beforeSend:function(){
							//$loader_mostrar.show();
						},
					...
					createdRow:function(nRow, aData, iDisplayIndex){
						...
						$('td', nRow).eq(7).addClass('text-nowrap text-right').attr('data-total',aData[7]);
