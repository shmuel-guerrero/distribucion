...
$ventas = $db->select('i.*, a.almacen, a.principal, e.nombres, e.paterno, e.materno')
    ...
    ->where('i.tipo', 'Venta')->where('i.anulado!=', '3')->where('i.nro_autorizacion!=', '')->where('i.fecha_egreso >= ', $fecha_inicial)->where('i.fecha_egreso <= ', $fecha_final)->order_by('i.fecha_egreso desc, i.hora_egreso desc')->fetch();
...
<div class="panel-heading" data-formato="<?= strtoupper($formato_textual); ?>" data-mascara="<?= $formato_numeral; ?>" data-gestion="<?= date_decode($gestion_base, $_institution['formato']); ?>">
	<h3 class="panel-title">
		...
		<strong>Reporte de ventas manuales</strong>
	</h3>
</div>
...
$(function () {
	...
	$.validate({
		...
		onSuccess: function () {
			...
			window.location = '?/reportes/ventas_manuales' + inicial_fecha + final_fecha;
		}
	});
	...
	<?php if ($ventas) { ?>
        var table = $('#table').on('draw.dt', function () { // search.dt order.dt page.dt length.dt
    		...
    		filter: true,
    		name: 'Reporte de ventas manuales',
