...
$ventas = $db->select('i.*, a.almacen, a.principal, e.nombres, e.paterno, e.materno')
    ...
    ->where('i.tipo', 'Venta')->where('i.anulado!=','3')->where('i.codigo_control != ', '')->where('i.fecha_egreso >= ', $fecha_inicial)->where('i.fecha_egreso <= ', $fecha_final)->order_by('i.fecha_egreso desc, i.hora_egreso desc')->fetch();
...
$(function () {
	...
	$.validate({
		...
		onSuccess: function () {
			...
			window.location = '?/reportes/ventas_electronicas' + inicial_fecha + final_fecha;
		}
	});
	...
	<?php if ($ventas) { ?>
        var table = $('#table').on('draw.dt', function () { // search.dt order.dt page.dt length.dt
    		...
    		filter: true,
    		name: 'Reporte de ventas electrónicas',
