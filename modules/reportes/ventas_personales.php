...
<div class="panel-heading" data-formato="<?= strtoupper($formato_textual); ?>" data-mascara="<?= $formato_numeral; ?>" data-gestion="<?= date_decode($gestion_base, $_institution['formato']); ?>">
	<h3 class="panel-title">
		...
		<strong>Reporte de ventas personales</strong>
	</h3>
</div>
	...
	<table id="table" class="table table-bordered table-condensed table-striped table-hover table-xs">
		...
		<tbody>
			...
			<tr>
				...
				<td class="text-nowrap"><?= escape($venta['tipo'] . ' ' . (($venta['codigo_control'] != '') ? 'electrónica' : 'manual')); ?></td>
...
$(function () {
	...
	$.validate({
		...
		onSuccess: function () {
			...
			window.location = '?/reportes/ventas_personales' + inicial_fecha + final_fecha;
		}
	});
	...
	<?php if ($ventas) { ?>
        var table = $('#table').on('draw.dt', function () { // search.dt order.dt page.dt length.dt
    		...
    		filter: true,
    		name: 'Reporte de ventas personales',
