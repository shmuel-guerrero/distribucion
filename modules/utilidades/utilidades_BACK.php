				...
				<?php foreach ($ventas as $nro => $venta) { ?>
					...
					if ($cantidadTotal > $prodAc) {
						...
						if ($iUltimo) {
							...
							$ultimoSaldo = Fracciones($ultimoSaldo, $iUltimo['tamanio']);
					...
					$query .= " WHERE vd.producto_id='" . $venta['id_producto'] . "' AND v.fecha_egreso between '$fecha_inicial' and '$fecha_final' AND v.tipo='venta' and v.anulado = 0";
