...
$query = "SELECT *, SUM(cantidad)AS cantidadAcumul, SUM(precio*cantidad/IF(a.cantidad_unidad IS NULL, 1, a.cantidad_unidad)) AS importeAcumul,e.nombres,e.paterno,e.materno, p.codigo as codigop
            ...
            WHERE v.fecha_egreso between '$fecha_inicial' and '$fecha_final' AND v.tipo='Venta' AND v.provisionado = 'S' AND v.estadoe=0 
		...
		<table id="table" class="table table-bordered table-condensed table-striped table-hover table-xs">
			...
			<tbody>
				...
				<?php foreach ($ventas as $nro => $venta) { ?>
					...
					$ingreso_ultimo = $db->select('*, b.costo')->from('inv_ingresos a')->join('inv_ingresos_detalles b','a.id_ingreso = b.ingreso_id')->join('inv_productos p','b.producto_id = p.id_producto')->join('inv_unidades c','c.id_unidad = p.unidad_id')->where('b.producto_id',$venta['id_producto'])->order_by('a.fecha_ingreso','desc')->fetch_first();
					//var_dump($ingreso_ultimo);exit();
					$ultimaUnidad = $ingreso_ultimo['unidad'];
					$ultimoCosto = $ingreso_ultimo['costo'];
					$subtotal = $ultimoCosto * $cantidadTotal;
					$subtotal = number_format($subtotal, 2, ".", " ");
					$costoTotal = $ultimoCosto * $cantidadTotal;
					
					$detalleCompra = "<tr><td style='color:#f00;'>" . $cantidadTotal . "</td><td style='color:#f00;'><b>" . $ultimaUnidad . "</b> " . $ultimoCosto . " " . $moneda . "</td><td style='color:#f00;'>" . number_format($subtotal, '2', '.', '') . " " . $moneda . "</td></tr>";
					...
					$query = "SELECT u.unidad,  precio, SUM(cantidad) as cantidadXprecio ";
					...
					$query .= " WHERE vd.producto_id='" . $venta['id_producto'] . "' AND v.fecha_egreso between '$fecha_inicial' and '$fecha_final' AND v.tipo='venta' AND v.provisionado = 'S' AND v.estadoe = 0 ";
			...
			<?php if ($permiso_cambiar) { ?>
				...
				$.validate({
					...
					onSuccess: function() {
						...
						window.location = '?/utilidades/utilidades_general_notas_remision' + inicial_fecha + final_fecha;
					}
				});
