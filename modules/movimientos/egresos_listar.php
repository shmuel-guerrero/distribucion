...
// Obtiene los egresos
$egresos = $db->select("m.*, concat(e.nombres, ' ', e.paterno, ' ', e.materno) as empleado")->from('caj_movimientos m')->join('sys_empleados e', 'm.empleado_id = e.id_empleado', 'left')->where('m.tipo', 'e')->where('m.empleado_id', $_user['persona_id'])->between('m.fecha_movimiento', $fecha_inicial, $fecha_final)->order_by('m.fecha_movimiento desc, m.hora_movimiento desc')->fetch();
...
<div class="panel-heading">
	<h3 class="panel-title" data-header="true">
		...
		<strong>Egresos de efectivo</strong>
	</h3>
</div>
		...
		<div class="col-xs-6 text-right">
			<div class="btn-group">
				...
				<ul class="dropdown-menu dropdown-menu-right">
					...
					<li><a href="?/movimientos/egresos_crear" data-backdrop="static" data-keyboard="false"><span class="glyphicon glyphicon-plus"></span> Crear ingreso</a></li>
				</ul>
			</div>
		</div>
	</div>
		...
		<tbody>
			...
			<tr>
				...
				<td class="text-nowrap">
					...
					<a href="?/movimientos/egresos_modificar/<?= $egreso['id_movimiento']; ?>" data-toggle="tooltip" data-title="Modificar egreso"><i class="glyphicon glyphicon-edit"></i></a>
					<?php endif ?>
					<?php if ($permiso_eliminar) : ?>	
					<a href="?/movimientos/egresos_eliminar/<?= $egreso['id_movimiento']; ?>" data-toggle="tooltip" data-title="Eliminar egreso" data-eliminar="true"><i class="glyphicon glyphicon-trash"></i></a>
					<?php endif ?>
				</td>
				<?php endif ?>
			</tr>
