...
if (sizeof($params) == 2) {
	...
	if (!is_date($params[0]) || !is_date($params[1])) {
		...
		redirect('?/movimientos/egresos_listar/' . $fecha_inicial . '/' . $fecha_final);
	}
} else {
	// Redirecciona la pagina
	redirect('?/movimientos/egresos_listar/' . $fecha_inicial . '/' . $fecha_final);
...
// Obtiene los egresos
$egresos = $db->select("m.*, upper(IFNULL(a.nombre, 'sucursal no registrado')) AS sucursal, concat(e.nombres, ' ', e.paterno, ' ', e.materno) as empleado")
	->from('caj_movimientos m')
	->join('sys_empleados e', 'm.empleado_id = e.id_empleado', 'left')
	->join('sys_instituciones a', 'm.sucursal_id = a.id_institucion', 'left')
	->where('m.tipo', 'e')->where('m.empleado_id', $_user['persona_id'])
...
$permiso_crear = in_array('egresos_crear', $permisos);
$permiso_modificar = in_array('egresos_modificar', $permisos);
$permiso_eliminar = in_array('egresos_eliminar', $permisos);
$permiso_imprimir = in_array('egresos_imprimir', $permisos);
		...
		<div class="col-xs-6 text-right">
			<div class="btn-group">
				...
				<ul class="dropdown-menu dropdown-menu-right">
					...
					<li><a href="?/movimientos/egresos_crear" target="_self" data-toggle="modal" data-keyboard="false"><span class="glyphicon glyphicon-plus"></span> Nuevo</a></li>
				</ul>
			</div>
		</div>
	</div>
		...
		<table id="table" class="table table-bordered table-condensed table-striped table-hover">
			...
			<tfoot>
				<tr class="active">
					...
					<th class="text-nowrap text-middle" data-datafilter-filter="true">Sucursal</th>
			...
			<tbody>
				<?php foreach ($egresos as $nro => $egreso) : ?>
					<tr>
						...
						<td class="text-nowrap"><?= escape($egreso['sucursal']); ?></td>
						...
						<td class="text-nowrap text-right" data-monto="<?= $egreso['monto']; ?>"><?= _toFixed($egreso['monto']); ?></td>
						<td><?= escape($egreso['observacion']); ?></td>
						<td class="text-nowrap"><?= escape($egreso['empleado']); ?></td>
						<?php if ($permiso_modificar || $permiso_eliminar || $permiso_imprimir) : ?>
							<?php $limite = date("Y-m-d", strtotime($egreso['fecha_movimiento'] . "+ 1 month")); ?>
							<td class="text-nowrap">
								<?php if ($permiso_modificar && date('Y-m-d') <= $limite) : ?>
									<a href="?/movimientos/egresos_modificar/<?= $egreso['id_movimiento']; ?>" data-toggle="tooltip" title="Puede eliminar el movimiento hasta la fecha: <?= $limite ?>"><i class="glyphicon glyphicon-edit"></i></a>
								<?php endif ?>
								<?php if ($permiso_eliminar && date('Y-m-d') <= $limite) : ?>
									<a href="?/movimientos/egresos_eliminar/<?= $egreso['id_movimiento']; ?>" data-toggle="tooltip" title="Puede eliminar el movimiento hasta la fecha: <?= $limite ?>" data-eliminar="true"><i class="glyphicon glyphicon-trash"></i></a>
								<?php endif ?>
								<?php if ($permiso_imprimir) : ?>
									<a href="?/movimientos/egresos_imprimir/<?= $egreso['id_movimiento']; ?>" data-toggle="tooltip" data-title="Imprimir comprobate" data-imprimir="true"><i class="glyphicon glyphicon-print"></i></a>
								<?php endif ?>
							</td>
						<?php endif ?>
					</tr>
...
<div id="modal_cambiar" class="modal fade" tabindex="-1">
	<div class="modal-dialog">
		<form method="post" action="?/movimientos/egresos_listar" id="form_cambiar" class="modal-content loader-wrapper" autocomplete="off">
	...
	$(function() {
		<?php if ($permiso_eliminar) : ?>
			$('[data-eliminar]').on('click', function(e) {
				...
				bootbox.confirm('Está seguro que desea eliminar el egreso?', function(result) {
					if (result) {
						window.location = url;
					}
				});
		...
		<?php if ($permiso_imprimir) : ?>
			$('[data-imprimir]').on('click', function(e) {
				e.preventDefault();
				var url = $(this).attr('href');
				bootbox.confirm('Está seguro que desea reemprimir el comprobante de egreso?', function(result) {
					if (result) {
			...
			$(window).bind('keydown', function(e) {
				if (e.altKey || e.metaKey) {
					switch (String.fromCharCode(e.which).toLowerCase()) {
						case 'n':
							e.preventDefault();
							window.location = '?/movimientos/egresos_crear';
							break;
					}
				}
			});
