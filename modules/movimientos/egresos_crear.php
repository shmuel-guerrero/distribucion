...
$comprobante = $db->query("select ifnull(max(nro_comprobante), 0) + 1 as nro_comprobante from caj_movimientos where tipo = 'e'")->fetch_first();
...
<div class="panel-heading">
	<h3 class="panel-title">
		...
		<strong>Crear nuevo egreso</strong>
	</h3>
</div>
<div class="panel-body">
	...
	<div class="row">
		<div class="col-sm-8 col-sm-offset-2">
			<!--<form method="post" action="?/movimientos/egresos_guardar" class="form-horizontal" autocomplete="off">-->
...
function guardar_egreso(form) {
	...
	if ($form.data('locked') != undefined && !$form.data('locked')) {
		...
		$.ajax({
		    type: 'post',
		    dataType: 'json',
		    url: '?/movimientos/egresos_guardar',
		    data: data,
		    beforeSend: function() {
		    ...
		    if (movimiento_id) {
		        $.notify({
		            message: 'El registro del egreso fue realizado satisfactoriamente.'
		        }, {
		            type: 'success'
		        });
		        ...
		        $('#loader').fadeOut(100);
		        $.notify({
		            message: 'Ocurri&oacute un problema en el proceso, no se puedo obtener los datos del egreso, verifique si el egreso se guard&oacute parcialmente.'
		        }, {
		            type: 'danger'
		        });
		    	$form.data('locked', false);
		    ...
		    $('#loader').fadeOut(100);
		    $.notify({
		        message: 'Ocurri&oacute un problema en el proceso, no se puedo obtener los datos del egreso, verifique si el egreso se guard&oacute parcialmenteeeeeeeeeee.'
		    }, {
		        type: 'danger'
		    });
		    $form.data('locked', false);
...
function imprimir_nota(movimiento_id) {
	window.open('?/movimientos/egresos_imprimir/' + movimiento_id, '_blank');
