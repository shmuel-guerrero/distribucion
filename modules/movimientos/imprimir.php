...
$ventas = $db->query("	SELECT * 
						FROM inv_egresos e 
						WHERE e.fecha_egreso = '$fecha' AND e.tipo='Venta' AND e.estado = 'V' AND e.empleado_id = '" . $_user['persona_id'] . "' 
						group by id_egreso")->fetch();
//Obtiene compras
$compras = $db->query("SELECT * FROM inv_ingresos i WHERE i.fecha_ingreso = '$fecha' AND i.tipo='Compra' AND i.empleado_id = '" . $_user['persona_id'] . "' group by id_ingreso")->fetch();
//cronograma de pagos
$cronogramas = $db->query("	select c.periodo, cc.detalle, cc.monto, cc.fecha_pago, cc.id_cronograma_cuentas, cc.tipo_pago 
							from cronograma c 
							left join cronograma_cuentas cc on c.id_cronograma = cc.cronograma_id 
							where cc.estado='1' and cc.empleado_id=" . $_user['persona_id'] . " and cc.fecha_pago='$fecha' 
							GROUP by c.id_cronograma")->fetch();
//COBRO DE VENTAS
$cobros = $db->query("	select p.id_pago, p.movimiento_id, pd.tipo_pago, pd.fecha_pago, e.nro_factura, e.fecha_egreso,e.nombre_cliente,e.nit_ci, e.monto_total, e.tipo, ifnull(monto,0) as subtotal 
						from inv_pagos p 
						left join inv_pagos_detalles pd on p.id_pago= pd.pago_id 
						LEFT JOIN inv_egresos e ON e.id_egreso=p.movimiento_id 
						where p.tipo='Egreso' and pd.estado='1' and pd.empleado_id=" . $_user['persona_id'] . " and pd.fecha_pago='$fecha' and e.fecha_egreso!=pd.fecha_pago
						")->fetch(); 
//pagos compras
$pagos_compras = $db->query("select p.id_pago, p.movimiento_id, i.fecha_ingreso, i.fecha_ingreso, i.tipo, i.id_ingreso, i.nombre_proveedor, 
							i.monto_total, pd.tipo_pago, ifnull(monto,0) as subtotal 
							from inv_pagos p 
							left join inv_pagos_detalles pd on p.id_pago= pd.pago_id 
							LEFT JOIN inv_ingresos i ON i.id_ingreso=p.movimiento_id 
							where p.tipo='Ingreso' and pd.estado='1' and pd.empleado_id=" . $_user['persona_id'] . " and pd.fecha_pago='$fecha' and i.fecha_ingreso!=pd.fecha_pago
							")->fetch();
...
class MYPDF extends TCPDF {
	public function Header() {
        ...
        // Page number
		$this->Cell(0, 0, 'CIERRE DE CAJA', 0, false, 'R', 0, '', 0, false, 'T', 'M');
		// Position at 15 mm from bottom
		$this->SetY(55);
		$this->SetX(180);
	...
	foreach ($ventas as $nro => $venta) {
		...
		if ($venta['plan_de_pagos'] == 'si'){ 
			...
			if ($venta['tipo_de_pago'] == 'Efectivo') {
				...
				$body_ventas .= '<td class="left-right" align="right">' . number_format($venta['monto_total'], 2, '.', '') . '</td>';
				$body_ventas .= '<td class="left-right" align="right">' . number_format($venta['monto_total'], 2, '.', '') . '</td>';
				$body_ventas .= '<td class="left-right" align="right">' . $venta['tipo_pago'] . '</td>';											
			}else{
				$total_venta_banco = $total_venta_banco + $venta['monto_total'];
				$body_ventas .= '<td class="left-right" align="right">' . number_format($venta['monto_total'], 2, '.', '') . '</td>';
				$body_ventas .= '<td class="left-right" align="right">' . number_format($venta['monto_total'], 2, '.', '') . '</td>';								
...
// Establece la fuente del titulo
	...
	$xxx=number_format(($total_ingresos + $total_venta) - ($total_egresos + $total_compra + $total_gasto) , 2, '.', '');

// Titulo del documento
	//$pdf->Cell(0, 10, $total_ingresos." + ".$total_venta." + ".$total_egresos." + ".$total_compra." + ".$total_gasto, 0, true, 'L', false, '', 0, false, 'T', 'M');
	$pdf->Cell(0, 10, 'TOTAL EFECTIVO: '.$xxx, 0, true, 'L', false, '', 0, false, 'T', 'M');
// Establece la fuente del contenido
	$pdf->SetFont(PDF_FONT_NAME_DATA, '', 7);
	// Salto de linea
	$pdf->Ln(3);
