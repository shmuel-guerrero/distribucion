...
$ingresos = $db->select("m.*, 
						upper(IFNULL(a.nombre, 'sucursal no registrado')) AS sucursal, 
	...
	->where('m.tipo', 'i')
	->where('m.empleado_id', $_user['persona_id'])
...
$permiso_imprimir = in_array('ingresos_imprimir', $permisos);
		...
		<div class="col-xs-6 text-right">
			<div class="btn-group">
				...
				<ul class="dropdown-menu dropdown-menu-right">
					...
					<li><a href="?/movimientos/ingresos_crear" target="_self" data-toggle="modal" data-keyboard="false"><span class="glyphicon glyphicon-plus"></span> Nuevo</a></li>
				</ul>
			</div>
		</div>
	</div>
			...
			<tbody>
				<?php foreach ($ingresos as $nro => $ingreso) : ?>
					<tr>
						...
						<td class="text-nowrap"><?= escape($ingreso['sucursal']); ?></td>
						...
						<td class="text-nowrap text-right" data-monto="<?= $ingreso['monto']; ?>"><?= _toFixed($ingreso['monto']); ?></td>
						<td><?= escape($ingreso['observacion']); ?></td>
						<td class="text-nowrap"><?= escape($ingreso['empleado']); ?></td>
						<?php if ($permiso_modificar || $permiso_eliminar || $permiso_imprimir) : ?>
							<td class="text-nowrap">
								<?php $limite = date("Y-m-d", strtotime($ingreso['fecha_movimiento']."+ 1 month")); ?>
								<?php if ($permiso_modificar && date('Y-m-d') <= $limite) : ?>
									<a href="?/movimientos/ingresos_modificar/<?= $ingreso['id_movimiento']; ?>" data-toggle="tooltip" title="Puede editar el movimiento hasta la fecha: <?= $limite ?>"><i class="glyphicon glyphicon-edit"></i></a>
								<?php endif ?>
								<?php if ($permiso_eliminar && date('Y-m-d') <= $limite) : ?>
									<a href="?/movimientos/ingresos_eliminar/<?= $ingreso['id_movimiento']; ?>" data-toggle="tooltip" title="Puede eliminar el movimiento hasta la fecha: <?= $limite ?>" data-eliminar="true"><i class="glyphicon glyphicon-trash"></i></a>
								<?php endif ?>
								<?php if ($permiso_imprimir) : ?>
									<a href="?/movimientos/ingresos_imprimir/<?= $ingreso['id_movimiento']; ?>" data-toggle="tooltip" title="Imprimir comprobate" data-imprimir="true"><i class="glyphicon glyphicon-print"></i></a>
								<?php endif ?>
							</td>
						<?php endif ?>
					</tr>
	...
	$(function() {
		...
		<?php if ($permiso_imprimir) : ?>
			$('[data-imprimir]').on('click', function(e) {
				e.preventDefault();
				var url = $(this).attr('href');
				bootbox.confirm('Está seguro que desea reemprimir el comprobante de ingreso?', function(result) {
					if (result) {
