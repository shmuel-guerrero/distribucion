...
// Obtiene la fecha
 ...
 $abrir = '';
 $num = $sw = 0;
 $ultimo_registro = $db->query("SELECT MAX(fecha) AS fecha FROM inv_caja")->fetch_first(); 
 ...
 if ($ultimo_registro) {
 	...
 	while ($fecha_ultimo_registro <  $fecha_actual && $sw == 0) {
		...
		// Obtiene las ventas
 		$ventas = $db->query("SELECT * FROM inv_egresos e WHERE e.fecha_egreso = '$fecha' AND e.tipo='Venta' AND e.empleado_id = " . $_user['persona_id'] . " group by id_egreso")->fetch();
		//Obtiene compras
		...
		// Obtiene los gastos
 		...
 		$fecha_n = '';
 		$fecha_n = $db->query("SELECT * FROM `inv_caja` WHERE fecha = (SELECT MAX(fecha) AS fecha FROM inv_caja)")->fetch_first(); 	
 		...
 		$dia_previo = strtotime("-1 day",strtotime(date('d-m-Y'))); 
 		if ($dia_previo == $fecha_ultimo_registro) {
 			$sw = 1;
 		}else{
 			$fecha_ultimo_registro = '';
 			$fecha_n = $db->query("SELECT MAX(fecha) AS fecha FROM inv_caja")->fetch_first(); 
 			$fecha_ultimo_registro = strtotime(date_decode($fecha_n['fecha'],$_institution['formato']));
 		}
 	} 

 	$fecha_ultimo_registro = strtotime($fecha_ultimo_registro);

 	if ($fecha_ultimo_registro == $fecha_actual) {
 		if ($fecha_n['estado'] == 'CIERRE' && $abrir = 'abrir'){
 			$estado = 'INICIO'; 			
 			$total_de_totales_ingreso = $total_de_totales_egreso = $total_de_totales_saldo = $total_de_totales = 0;		
 			$fecha = (isset($_POST['fecha'])) ? date_encode($_POST['fecha']):date('Y-m-d');
 		}
 	}
 	$fecha_n = '';
 	$fecha_n = $db->query("SELECT * FROM `inv_caja` WHERE fecha = (SELECT MAX(fecha) AS fecha FROM inv_caja)")->fetch_first(); 	
