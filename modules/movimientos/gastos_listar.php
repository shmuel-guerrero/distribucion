...
if (sizeof($params) == 2) {
	...
	if (!is_date($params[0]) || !is_date($params[1])) {
		...
		redirect('?/movimientos/gastos_listar/' . $fecha_inicial . '/' . $fecha_final);
	}
} else {
	// Redirecciona la pagina
	redirect('?/movimientos/gastos_listar/' . $fecha_inicial . '/' . $fecha_final);
...
// Obtiene los gastos
$gastos = $db->select("m.*, concat(e.nombres, ' ', e.paterno, ' ', e.materno) as empleado")->from('caj_movimientos m')->join('sys_empleados e', 'm.empleado_id = e.id_empleado', 'left')->where('m.tipo', 'g')->where('m.empleado_id', $_user['persona_id'])->between('m.fecha_movimiento', $fecha_inicial, $fecha_final)->order_by('m.fecha_movimiento desc, m.hora_movimiento desc')->fetch();

// Obtiene los permisos
$permisos = explode(',', permits);

// Almacena los permisos en variables
$permiso_crear = in_array('gastos_crear', $permisos);
$permiso_modificar = in_array('gastos_modificar', $permisos);
$permiso_eliminar = in_array('gastos_eliminar', $permisos);
$permiso_imprimir = in_array('gastos_imprimir', $permisos);
...
<div class="panel-body">
		...
		<div class="col-xs-6 text-right">
			<div class="btn-group">
				...
				<ul class="dropdown-menu dropdown-menu-right">
					...
					<li><a href="?/movimientos/gastos_crear" target="_self" data-toggle="modal" data-keyboard="false"><span class="glyphicon glyphicon-plus"></span> Nuevo</a></li>
				</ul>
			</div>
		</div>
	</div>
		...
		<table id="table" class="table table-bordered table-condensed table-striped table-hover">
			<thead>
				<tr class="active">
					...
					<?php if ($permiso_modificar || $permiso_eliminar || $permiso_imprimir) : ?>
			...
			<tbody>
				<?php foreach ($gastos as $nro => $gasto) : ?>
					<tr>
						...
						<td class="text-nowrap"><?= escape($gasto['nro_comprobante']); ?></td>
						<td class="text-nowrap">
							<span><?= escape(date_decode($gasto['fecha_movimiento'], $_institution['formato'])); ?></span>
							<span class="text-primary"><?= escape($gasto['hora_movimiento']); ?></span>
						</td>
						<td><?= escape($gasto['concepto']); ?></td>
						<td class="text-nowrap text-right" data-monto="<?= $gasto['monto']; ?>"><?= _toFixed($gasto['monto']); ?></td>
						<td><?= escape($gasto['observacion']); ?></td>
						<td class="text-nowrap"><?= escape($gasto['empleado']); ?></td>
						<?php if ($permiso_modificar || $permiso_eliminar || $permiso_imprimir) : ?>
							<td class="text-nowrap">
								<?php $limite = date("Y-m-d", strtotime($gasto['fecha_movimiento']."+ 1 month")); ?>
								<?php if ($permiso_modificar && date('Y-m-d') <= $limite) : ?>
									<a href="?/movimientos/gastos_modificar/<?= $gasto['id_movimiento']; ?>" data-toggle="tooltip" title="Puede eliminar el movimiento hasta la fecha: <?= $limite ?>" ><i class="glyphicon glyphicon-edit"></i></a>
								<?php endif ?>
								<?php if ($permiso_eliminar && date('Y-m-d') <= $limite) : ?>
									<a href="?/movimientos/gastos_eliminar/<?= $gasto['id_movimiento']; ?>" data-toggle="tooltip" title="Puede eliminar el movimiento hasta la fecha: <?= $limite ?>"  data-eliminar="true"><i class="glyphicon glyphicon-trash"></i></a>
								<?php endif ?>
								<?php if ($permiso_imprimir) : ?>
									<a href="?/movimientos/gastos_imprimir/<?= $gasto['id_movimiento']; ?>" data-toggle="tooltip" data-title="Imprimir comprobate" data-imprimir="true"><i class="glyphicon glyphicon-print"></i></a>
								<?php endif ?>
							</td>
						<?php endif ?>
					</tr>
	...
	<?php else : ?>
		<div class="alert alert-info margin-none">
			...
			<ul>
				<li>No existen gastos registrados.</li>
				<li>Puede buscar gastos por rango de fechas, verifique que las fechas sean válidas.</li>
			</ul>
		</div>
	<?php endif ?>
</div>
...
<div id="modal_cambiar" class="modal fade" tabindex="-1">
	<div class="modal-dialog">
		<form method="post" action="?/movimientos/gastos_listar" id="form_cambiar" class="modal-content loader-wrapper" autocomplete="off">
	...
	$(function() {
		<?php if ($permiso_eliminar) : ?>
			$('[data-eliminar]').on('click', function(e) {
				...
				bootbox.confirm('Está seguro que desea eliminar el gasto?', function(result) {
					if (result) {
						window.location = url;
					}
				});
		...
		<?php if ($permiso_imprimir) : ?>
			$('[data-imprimir]').on('click', function(e) {
				e.preventDefault();
				var url = $(this).attr('href');
				bootbox.confirm('Está seguro que desea reemprimir el comprobante de gasto?', function(result) {
					if (result) {
			...
			$(window).bind('keydown', function(e) {
				if (e.altKey || e.metaKey) {
					switch (String.fromCharCode(e.which).toLowerCase()) {
						case 'n':
							e.preventDefault();
							window.location = '?/movimientos/gastos_crear';
							break;
					}
				}
			});
		...
		var $modal_cambiar = $('#modal_cambiar'),
			$form_cambiar = $('#form_cambiar'),
			$loader_cambiar = $('#loader_cambiar'),
			$inicial_cambiar = $('#inicial_cambiar'),
			$final_cambiar = $('#final_cambiar');
		...
		$.validate({
			...
			onSuccess: function() {
				...
				var direccion_cambiar = $.trim($form_cambiar.attr('action')),
					inicial_cambiar = $.trim($inicial_cambiar.val()),
					final_cambiar = $.trim($final_cambiar.val());
