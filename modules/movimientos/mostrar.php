...
$ultimo_registro_caja = $db->query("
	SELECT * 
	FROM `inv_caja` 
	WHERE fecha = (SELECT MAX(fecha) AS fecha FROM inv_caja) 
	AND hora_caja = (SELECT MAX(hora_caja) AS fecha FROM inv_caja) 
	AND id_caja = (SELECT MAX(id_caja) AS fecha FROM inv_caja)
")->fetch_first();
$estado = '';
...
<script>
	...
	async function loadSumaAllDataTables() {
		...
		let ingresos = await loadDataTableIngresos(fecha, true);
		let ventas = await loadDataTableVentas(fecha, true);
		let egresos = await loadDataTableEgresos(fecha, true);
		let compras = await loadDataTableCompras(fecha, true);
		let gastos = await loadDataTableGastos(fecha, true);
