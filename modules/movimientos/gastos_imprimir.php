...
$movimiento = $db->query("SELECT m.*, ifnull(concat( e.nombres, ' ', e.paterno, ' ', e.materno ), '-') as autorizado_por, ifnull(concat( er.nombres, ' ', er.paterno, ' ', er.materno ), '-') as recibido_por
...
// ------------------------------------------------------------
	...
	$pdf->Cell(0, 10, 'COMPROBANTE DE GASTO NRO. ' . $nro_comprobante, 0, true, 'C', false, '', 0, false, 'T', 'M');
    ...
    $pdf->SetFont($font_name_data, 'B', $font_size_data);
    $pdf->Cell($width_table * 0.16, $height_cell, 'FECHA AUTORIZACION:', 0, 0, 'L', 0, '', 1, true, 'T', 'M');
    $pdf->SetFont($font_name_data, '', $font_size_data);
...
$tabla .=<<<EOD
<tr>
    <th width="20%" class="all"><b>Observación: </b></th>
