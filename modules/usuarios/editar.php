		...
		<div class="col-sm-8 col-sm-offset-2">
			<form method="POST" action="?/usuarios/guardar" class="form-horizontal">
				...
				<div class="form-group">
					...
					<div class="col-md-9">
						<input type="text" value="<?= $user['email']; ?>" name="email" id="email" class="form-control" autocomplete="off" data-validation="email required">
					</div>
				</div>
				<div class="form-group">
				...
				<div class="form-group">
                    <label for="id_almacen" class="col-sm-3 control-label">Almacen:</label>
                    <div class="col-sm-9">
				...
				// $almacenes = $db->from('inv_almacenes')->where('principal','S')->fetch();
                            foreach ($almacenes as $nro => $almacen) {
                                if($user['almacen_id']==$almacen['id_almacen']){ ?>
                                    <option value="<?= $almacen['id_almacen'] ?>" selected><?= $almacen['almacen'] ?></option>
                                    <?php }else{?>
