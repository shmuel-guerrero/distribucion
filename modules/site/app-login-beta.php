...
if(is_post()) {
    ...
    if (isset($_POST['username']) && isset($_POST['password']) && isset($_POST['imei']) && isset($_POST['model'])) {
        ...
        try {
            ...
            $usuario = $db->query("select id_user, email, a.persona_id, a.avatar, b.nombres, b.paterno, b.materno, b.genero, b.telefono, b.fecha, a.rol_id 
                                    FROM sys_users a 
            ...
            $id_device = $db->query("SELECT * FROM sys_users_devices WHERE user_id = '{$usuario['id_user']}'")->fetch_first();

            if (!$id_device) {
                //se cierra transaccion
				$db->commit();
                echo json_encode(array('estado' => 'n',
                                        'msg' => (!$usuario) ? 'Datos erroneos.' : 'Dispositivo no registrado; edite y guarde en la web su usuario.'));
                exit;
            }else if(!$id_device['token'] && !$id_device['imei']) {

                //if ($usuario['email']) {
                if (!$id_device['token'] && !$id_device['imei']) {

                    $token_device = sha1(prefix . md5($usuario['email'] . $model . $imei));
                    $datos = array('token' => $token_device, 'imei' => $imei, 'model' => $model);
                    
                    //verifica si el imei y modelo estan registrado en la bas e datos
                    $validar_dispositivo = $db->query("SELECT * FROM sys_users_devices WHERE imei = '{$imei}' 
                                                        and model = '{$model}'")->fetch_first();


                    if (!$validar_dispositivo) {
                        $estado_device = 'Registrado';
                        // se guarad el nuevo token
                        $db->where(array('user_id' => $usuario['id_user']))->update('sys_users_devices', $datos);
                        $token_obtenido = $db->query("SELECT * FROM sys_users_devices WHERE user_id = {$usuario['id_user']}")->fetch_first()['token'];                           
                    
                    }else if($imei && $model){

                        $token = $db->query("SELECT * FROM sys_users_devices 
                                        WHERE imei = '{$imei}' AND model = '{$model}' ")->fetch_first();  

                        
                        if ($token['tipo'] == 'Premium') { 

                            $cantidad_disp = $db->query("SELECT COUNT(*) AS nro_dispositivos FROM sys_user_devices_detalles
                                                    WHERE device_id = '{$token['id_device']}'")->fetch_first()['nro_dispositivos'];

                            $validar_registro = $db->query("SELECT * FROM sys_users_devices_detalles dd 
                                                    WHERE dd.model = '$model' AND dd.imei = '$imei'")->fetch_first();
                            
                            if ($cantidad_disp <= $token['cant_user'] && $usuario['id_user'] && !$validar_registro) {
                                ...
                                $datos = array(
                                    ...
                                    'device_id' => $token['id_device']
                                );
                                $estado_device = 'Registrado';
                                // se guarad el nuevo token
                                $id_dispositivo = $db->insert('sys_users_devices_detalles', $datos);
                                
                                $token = $db->from('sys_users_devices_detalles')->where('id_detalle', $id_dispositivo)->fetch_first()['token'];

                                $token_obtenido = $token;

                            }else if($validar_registro){
                                $estado_device = 'Session';  
                                $token_obtenido = $validar_registro['token'];
                            }else{
                                //se cierra transaccion
                                $db->commit();
                                ...
                                echo json_encode(array('estado' => 'n',
                                                        ...
                                                        'msg' => '2.El usuario esta ligado a otro dispositivo. Acción restringida.'));
                                exit;
                            }
                        }else{
                            //se cierra transaccion
                            ...
                            echo json_encode(array('estado' => 'n',
                                                    'estado_device' => $estado_device,
                                                    'msg' => 'El usuario esta ligado a dispositivo con plan Basico.'));
                            exit;
                        }
                        ...
                        echo json_encode(array('estado' => 'n',
                                                'estado_device' => $estado_device,
                                                'msg' => '1.El usuario esta ligado a otro dispositivo.'));
                        exit;
                    }
                    ...
                    echo json_encode(array('estado' => 'n',
                                            'estado_device' => $estado_device,
                                            'msg' => 'No tiene registrado e-mail.'));
                    exit;
                }
            }else {
                $token = $db->query("SELECT * FROM sys_users_devices WHERE user_id = '{$usuario['id_user']}' ")->fetch_first();  
                
                if ($imei == $token['imei'] && $model == $token['model']) {
                    $token = $token['token'];
                    $estado_device = 'Session';                              
                }else if($imei && $model){
                        ...
                        if ($token['tipo'] == 'Premium') { 
                            ...
                            echo json_encode(array('estado' => 'n',
                                                    'estado_device' => $estado_device,
                                                    'msg' => 'El usuario esta ligado a dispositivo con plan Basico. Acción restringida.'));
                            exit;
                        }
                        ...
                        echo json_encode(array('estado' => 'n',
                                                'estado_device' => $estado_device,
                                                'msg' => '3.El usuario esta ligado a otro dispositivo. Acción restringida.'));
                        exit;
                    }
            }
