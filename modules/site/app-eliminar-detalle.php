		...
		try {
            ...
            if($detalle){
                if ($detalle['promocion_id'] != 0 && $detalle['promocion_id'] != '') {
                     ...
                     */
                    $id_movimiento = registros_historial($db, '_eliminar_previo', 'inv_egresos', 'id_egreso', $egreso['id_egreso'], '', '', $id_user, 'SI', 0);
                    $movimiento = registros_historial($db, '_eliminar_previo', 'inv_egresos_detalles', 'id_detalle', $id_detalle, '', '', $id_user, 'NO', $id_movimiento);

                    //se crea backup de registros a eliminar
                    $verifica_id = backup_registros($db, 'inv_egresos', 'id_egreso', $detalle['egreso_id'], '', '', $empleado['persona_id'], 'SI', 0, "Eliminado");
                    ...
                    if ($registros > 1) {
                        ...
                        $db->where('id_egreso', $egreso['id_egreso'])->update('inv_egresos', array('monto_total' => $monto_total,'monto_total_descuento' => ($monto_total - $egreso['descuento_bs']),'descuento_bs' => $egreso['descuento_bs'], 'nro_registros' => ($registros - 1)));
                    } else {
                    ...
                    if ($credito == '1' || $credito = 1) {
                        ...
                        if ($creditoEx){   
                            //se crea backup de registros a eliminar
                            $verifica_id = backup_registros($db, 'inv_pagos', 'movimiento_id', $egreso['id_egreso'], '', '', $empleado['persona_id'], 'SI', 0, "Eliminado");
                            // se eliminan registros                         
