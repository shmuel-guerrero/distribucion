...
function printer_center($quantity = '', $detail = '', $price = '', $amount = '', $codes1 = '')
{
    ...
    $price    = ' ' . $price . ' ';
    $amount   = ' ' . $amount . ' ';
    $quantity = str_pad($quantity, $column - 4, ' ', STR_PAD_LEFT);
    $code1 	  = str_pad($codes1, $column - 4, ' ', STR_PAD_LEFT); // Josema
    $detail   = str_pad($detail, $full_width - ($column * 3), ' ', STR_PAD_RIGHT);
    $price    = str_pad($price, $column - 4, ' ', STR_PAD_LEFT);
    $amount   = str_pad($amount, $column , ' ', STR_PAD_LEFT);
    return $quantity . $code1 . $detail . $price . $amount;
}
...
function generar_zpl($datos)
{
    ...
    /**
     * Imprime codigo de cliente de emision de la factura
     */
    $fecha_hora           = printer_justify($factura_fecha,$factura_hora);
    /**
    ...
    foreach ($venta_cantidades as $key => $cantidad) {
        ...
        $res = printer_center($cantidad,$detalle,$precio,$subtotal,$code);
     ...
     * Imprime el total de la venta
     */
//    $pie2                = printer_justify(' ' . $venta_total_titulo,$venta_total_numeral . ' ');
//    $temC                = $jump;
    /**
     * Imprime el total de la venta
     */
//    $pie3                = printer_justify(' ' . $venta_total_titulo,$venta_total_numeral . ' ');
//    $temC                = $jump;
    /**
     * Dibuja una linea
     */
...
//    $pie2 = "^FT0,$lineat^AcN,$altoLetra,$anchoLetra^FH\^FD$pie2^FS";
     ...
     * Definimos la posicion en que se imprimira el monto total de la venta
     */
//    $pie3 = "^FT0,$lineat^AcN,$altoLetra,$anchoLetra^FH\^FD$pie3^FS";
    /**
     * Imprime el nombre de la empresa
     */
	...
	^LL$jump
	^LH0,0
	^FS".$imagen.$empresa_nombre.$empresa_sucursal.$empresa_direccion.$empresa_telefono.$empresa_ciudad.$factura_titulo.$linea1.$empresa_nit.$factura_numero.$factura_autorizacion.$linea2.$empresa_actividad.$fecha_hora.$cliente_nit.$cliente_nombre.$cabecera.$tabla.$linea_cab.$linea_cab2.$linea_cab3.$pie.$linea_cab4.$verticalL.$verticalR.$verticalC1.$verticalC2.$verticalC3.$verticalC4.$venta_total_literal.$linea_cab5.$factura_codigo.$factura_limite.$qr.$factura_autenticidad.$factura_leyenda.$linea3.$factura_vendedor.$factura_agradecimiento."^XZ";
...
if (is_post()) {
    ...
    if (isset($_POST['id_cliente'])) {
        ...
        if($_POST['factura'] == 'si'){
            ...
            $nro_factura = 'Nro FACT.: ' . $venta['nro_factura'];
            //almacen
            ...
            if($dosificacion){
                ...
                $nro_factura = 'Nro DE VENTA: ' . $venta['nro_factura'];
                $codigo_control = '';
                $fecha_limite = '';
                $factura_autenticidad = '';
                $factura_leyenda = '';
        ...
        if (true) {
            ...
            if ($venta['id_egreso']) {
                ...
                $datos = array(
                    ...
                    'venta_titulos' => array('CANT.', ' DETALLE', 'P. UNIT.', 'SUBT.   ', 'COD.'),
