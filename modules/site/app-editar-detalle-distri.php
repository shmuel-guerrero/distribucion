...
if (is_post()) {
    ...
    if (isset($_POST['id_user']) && isset($_POST['id_detalle']) && isset($_POST['cantidad']) && isset($_POST['unidad_id']) && $_POST['cantidad'] != 0) {
        ...
        try {
            ...
            //validar si es producto con promocion
            if($detalle['promocion_id'] != 0 && $detalle['promocion_id'] != ''){
                ...
                if (validar_cambio_unidad($db, 'inv_egresos_detalles', $id_detalle, $cantidad, $id_unidad)) {                
                    ...
                    //if($cantidad_aValidar > $cantidad_detalle){
                        //se cierra transaccion
                        $db->commit();
                        echo json_encode(array('estado' => 'cantidad mayor'));
                        exit;
                    } else { // para mayor
                            ...
                            if($id_unidad == $detalle['unidad_id']){
                                if($cantidad < $cantidad_detalle){
                                    ...
                                    $db->where('id_egreso',$id_egreso)->update('inv_egresos',array('monto_total' => $monto_total, 'monto_total_descuento' => ($monto_total - $egreso['descuento_bs'])));
                                    $db->where(array('id_egreso'=>$id_egreso, 'distribuidor_estado'=>'ENTREGA'))->update('tmp_egresos',array('monto_total' => $monto_total, 'monto_total_descuento' => ($monto_total - $egreso['descuento_bs'])));
                                    ...
                                    $egreso['estado'] = 3;
                                    $egreso['accion'] = 'VentaDevuelto';
                                    $id_tmp = $db->insert('tmp_egresos', $egreso);
                
                                    $detalle['tmp_egreso_id'] = $id_tmp;
                            ...
                            $id_movimiento = registros_historial($db, '_editar_post', 'inv_egresos', 'id_egreso', $id_egreso, '', '', $id_user, 'SI', 0);
                            $movimiento = registros_historial($db, '_editar_post', 'inv_egresos_detalles', 'id_detalle', $id_detalle_historial, '', '', $id_user, 'NO', $id_movimiento);
                    ...
                    $db->commit();
                    echo json_encode(array('estado' => 'n',
                                            'msg' => 'Es una cantidad mayor al stock. Verifique que la unidad se de menor o igual tamanio al actual'));
                }
            }
        } catch (Exception $e) {
            $status = false;
