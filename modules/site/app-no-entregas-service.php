    ...
    */                            
    ...
    if (count($egresos_mov) > 0 && $egresos_mov['id_egresos'] != null && $egresos_mov['id_productos'] != null) {
        ...
        $detalles = $db->query("SELECT e.id_egreso,
                        ...
                        FROM (SELECT DISTINCTROW(id_egreso) FROM tmp_egresos te WHERE te.distribuidor_id = {$distribuidor} AND te.distribuidor_estado NOT IN ('ENTREGA', 'VENTA') AND te.estado = 3) e 
       ...
       /*var_dump($detalles);
        ...
        $ventas_directas = $db->query('SELECT te.id_egreso,
                ...
                WHERE edi.egreso_id IS NOT NULL AND 
                te.estado = 3 AND te.distribuidor_id = ' . $distribuidor . ' AND te.distribuidor_estado IN ("VENTA") AND edi.promocion_id != 1 AND a.visible = "s"
        ...
        $resumen_entregas = $db->query("SELECT c.id_cliente AS codigo_cliente, c.cliente, c.nombre_factura, c.nit, IFNULL(e.monto_total, 0)AS monto_total, 
                            ...
                            WHERE  e.id_egreso IN ({$egresos_mov['id_productos']}) AND e.estadoe = 3 AND e.anulado = 0
        ...
        foreach ($resumen_entregas as $key => $value) {
            ...
            $resumen[$key]['monto3'] = number_format((($value['monto_cancelado'] >= 0) ? $value['monto_cancelado'] : 0), 2, '.', '');
        }
       ...
       /*  $datos_entregados = prepara_tabla($productos_entregados, "PRODUCTOS ENTREGADOS");
        $datos_devueltos = prepara_tabla($productos_devueltos, "PRODUCTOS DEVUELTOS - NO ENTREGADOS");
        $datos_ventas_directas = prepara_tabla($ventas_directas, "VENTAS DIRECTAS");
        $datos_resumen = prepara_tabla2($resumen, "RESUMEN DE MOVIMIENTOS");
        $datos_cobros = prepara_tabla2($cobros, "COBROS DEUDAS"); */
    }else{
        $productos_devueltos = array();
    }
}
...
function prepara_tabla($datos_array = array(), $titulo = "REPORTE", $valor_moneda = "")
{
    // Estructura la tabla
    $body = '';
    $total = 0;
    ...
    if (count($datos_array) > 0) {
        ...
        foreach ($datos_array as $nro => $detalle) {
            ...
            if ($detalle['cantidad'] > 0) {
                $body .= '<tr height="2%" >';
                $body .= '<td class="left-right bot" align="right">' . number_format((($detalle['cantidad']) ? $detalle['cantidad'] : 0), 0, '.', '') . '</td>';
