    ...
    if (isset($_POST['id_user']) && isset($_POST['id_cliente'])) {
        ...
        try {
            ...
            $productos = $db->query('SELECT DISTINCT b.*,c.id_producto, c.promocion, c.codigo, c.nombre_factura as nombre, c.precio_sugerido, c.precio_sugerido as stock, d.categoria, e.unidad, c.unidad_id AS unidad_idp, b.unidad_id AS unidad_ide, e.unidad AS total, a.id_egreso
                ...
                WHERE a.estadoe = 2 AND a.cliente_id = '. $cliente_id .' AND g.distribuidor_id = '. $emp .' AND g.estado = 1 AND a.grupo = "" AND b.promocion_id != 1 and (a.fecha_egreso <= w.fecha OR a.fecha_egreso < CURDATE()) ')->fetch();

            $productos2 = $db->query('SELECT DISTINCT b.*,c.id_producto, c.promocion, c.codigo, c.nombre_factura as nombre, c.precio_sugerido, c.precio_sugerido as stock, d.categoria, e.unidad, c.unidad_id AS unidad_idp, b.unidad_id AS unidad_ide, e.unidad AS total, a.id_egreso
                FROM gps_asigna_distribucion g
                ...
                LEFT JOIN inv_categorias d ON c.categoria_id = d.id_categoria
                LEFT JOIN inv_unidades e ON c.unidad_id = e.id_unidad
                WHERE a.estadoe = 2 AND a.cliente_id = '. $cliente_id .' AND g.distribuidor_id = '. $emp .' AND g.estado = 1 AND a.grupo != "" AND b.promocion_id != 1 AND a.fecha_egreso <= CURDATE()')->fetch();
            ...
            if($productos){
                foreach ($productos as $nro => $producto) {
                    if($producto['unidad_idp']!=$producto['unidad_ide']){
                        ...
                        $productos[$nro]['cantidad_unidad'] = $unidad['cantidad_unidad'];
                        ...
                        $productos[$nro]['cantidad_unidad'] = 1;
