...
if ($id_almacen == 0) {
	...
	$almacenes = $db->select('z.*')
	->from('inv_almacenes z')
	->order_by('z.id_almacen')
	->fetch();
} else {
...
if ($id_almacen == 0) {
	...
	$pdf->Cell(0, 10, 'LISTA DE ALMACENES', 0, true, 'C', false, '', 0, false, 'T', 'M');
	...
	foreach ($almacenes as $nro => $almacen) {
		$body .= '<tr>';
		$body .= '<td>' . ($nro + 1) . '</td>';
		$body .= '<td>' . escape($almacen['almacen']) . '</td>';
		$body .= '<td>' . escape($almacen['direccion']) . '</td>';
		$body .= '<td>' . escape($almacen['telefono']) . '</td>';
		$body .= '<td>' . ((escape($almacen['principal']) == 'S') ? 'Si' : 'No') . '</td>';
		$body .= '<td>' . escape($almacen['descripcion']) . '</td>';
		$body .= '</tr>';
	}
	
	$body = ($body == '') ? '<tr><td colspan="6" align="center">No existen almacenes registrados en la base de datos</td></tr>' : $body;
...
EOD;
	...
	$nombre = 'almacenes_' . date('Y-m-d_H-i-s') . '.pdf';
} else {
	...
	// Titulo del documento
	$pdf->Cell(0, 10, 'DETALLE DE ALMACéN', 0, true, 'C', false, '', 0, false, 'T', 'M');
	
	// Establece la fuente del titulo
	$pdf->SetFont('helvetica', 'I', 10);
	
	// Subtitulo del documento
	$pdf->Cell(0, 5, 'ID: ' . $id_almacen, 0, true, 'C', false, '', 0, false, 'T', 'M');
	...
	$valor_almacen = escape($almacen['almacen']);
	$valor_direccion = escape($almacen['direccion']);
	$valor_telefono = escape($almacen['telefono']);
	$valor_principal = (escape($almacen['principal']) == 'S') ? 'Si' : 'No';
	$valor_descripcion = escape($almacen['descripcion']);
	...
	<table cellpadding="1">
		<tr>
			...
			<td width="80%" style="border: 1px solid #444;">
				<table cellpadding="5">
					<tr>
						<th width="40%">Almacén:</th>
						<td width="60%">$valor_almacen</td>
					</tr>
					<tr>
						<th width="40%">Dirección:</th>
						<td width="60%">$valor_direccion</td>
					</tr>
					<tr>
					...
					<tr>
						...
						<td width="60%">$valor_principal</td>
					</tr>
					<tr>
...
EOD;
	...
	// Genera el nombre del archivo
	$nombre = 'almacen_' . $id_almacen . '_' . date('Y-m-d_H-i-s') . '.pdf';
}
