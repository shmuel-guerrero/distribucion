...
if ($id_egreso == 0) {
	...
	$detalles = $db->select('d.*, d.unidad_id as unidad_otra, p.codigo, p.nombre, p.nombre_factura, d.precio as precio_actual, m.marca')
...
if ($id_egreso == 0) {
	...
	$monto_total = escape($egreso['monto_total']);
	...
	$valor_total_descuento = escape($egreso['monto_total_descuento']);
	...
	foreach ($detalles as $nro => $detalle) {
		if($detalle['promocion_id'] != 1) {
			...
			$precio_actual = escape($detalle['precio_actual']);

			$importe = $cantidad * $precio_actual;
			$total = $total + $importe;
