...
//$Responsables = $db->query("SELECT e.id_empleado,e.nombres,e.paterno,e.materno FROM sys_empleados AS e INNER JOIN sys_users AS u ON u.persona_id=e.id_empleado WHERE rol_id='14'")->fetch();
$Fecha = date('Y-m-d');
$Responsables = $db->query("SELECT e.id_empleado,e.nombres,e.paterno,e.materno,
	...
	WHERE u.rol_id='14' GROUP BY e.id_empleado")->fetch();
//echo $db->last_query();
					...
					<form id="formulario" class="form-horizontal">
						<div style="zoom: 1;">
							<div class="form-group">
								...
								<div class="col-sm-8">
									<select name="responsable" id="responsable" class="form-control text-uppercase" required>
										<option value="" disabled selected>Buscar</option>
										<?php
										foreach ($Responsables as $Responsable) :
											if (!$Responsable['disponible']) :
										?>
												<option value='<?= $Responsable['id_empleado'] ?>'><?php echo "{$Responsable['nombres']} {$Responsable['paterno']} {$Responsable['materno']}"; ?></option>
										<?php
											endif;
										endforeach;
										?>
		...
		<div class="col-md-6">
			<div class="panel panel-default">
				...
				<div class="panel-body">
					...
					<?php endif ?>
					<form method="post" action="?/autoventas/buscar" id="form_buscar_0" class="margin-bottom" autocomplete="off">
						<input type='hidden' name='IdAlmacen' value='<?= $id_almacen ?>'>
					...
					<form method="post" action="?/autoventas/buscar" id="form_buscar_1" class="margin-bottom" autocomplete="off">
						<input type='hidden' name='IdAlmacen' value='<?= $id_almacen ?>'>
						<div class="form-group has-feedback">
...
<table class="hidden">
	<tbody id="fila_filtrar" data-negativo="<?= imgs; ?>/" data-positivo="<?= files; ?>/productos/">
		<tr onclick="desp(this)">
			...
			<td class="hidden" data-prestamo=""></td>
			<td class="hidden" data-stockp=""></td>
		</tr>
		<tr>
...
<script>
				...
				$.ajax({
					...
					if (productos.length) {
						...
						for (var i in productos) {
							...
							$ultimo.eq(9).attr('data-prestamo', productos[i].id_producto);
							$ultimo.eq(9).text(productos[i].prestamo);

							$ultimo.eq(10).attr('data-stockp', productos[i].id_producto);
							$ultimo.eq(10).text(productos[i].stockp);
							//console.log(productos[i].prestamo)
						}
	...
	function adicionar_producto(id_producto) {
		...
		var prestamo = $.trim($('[data-prestamo=' + id_producto + ']').text());
		var stockp = $.trim($('[data-stockp=' + id_producto + ']').text());
		var plantilla = '';
		var cantidad;
		var $modcant = $('#modcant');
		//console.log(aa);
	...
	async function ActualizarAutoVendedor() {
		const Consulta = await fetch('?/autoventas/buscar_auto_vendedor');
		const Datos = await Consulta.json();
		console.log(Datos);
		responsable.innerHTML = "<option value='' disabled selected>Buscar</option>";
		Datos.forEach(Dato => {
			if(parseInt(Dato['disponible'])==0)
				responsable.innerHTML += `<option value='${Dato['id_empleado']}'>${Dato['nombres']} ${Dato['paterno']} ${Dato['materno']}</option>`;
		});
	}
</script>
