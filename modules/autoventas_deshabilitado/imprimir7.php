...
$empleados2 = $db->query('SELECT *
    from tmp_egresos b 
    left join sys_empleados e on e.id_empleado = b.empleado_id 
    where (b.distribuidor_fecha  BETWEEN "'.$fecha_inicial.'"  AND "'.$fecha_final.'") AND
    b.distribuidor_id = '.$distribuidor.' group by b.empleado_id')->fetch();
 
foreach($empleados2 as $empleado2){
   $valor_empleado2 = $valor_empleado2.'<br>'.$empleado2['nombres'];
...
// Obtiene los empleados
/*$empleados = $db->select('w.id_empleado, w.nombres, w.paterno, w.materno, GROUP_CONCAT(a.ruta_id SEPARATOR "&") as emp')
                  ->from('gps_asigna_distribucion a')
                  ->join('gps_rutas z','a.ruta_id = z.id_ruta')
...
// Obtiene los detalles
    ...
    FROM tmp_egresos b 
    LEFT JOIN tmp_egresos_detalles c ON b.id_egreso = c.egreso_id
    LEFT JOIN inv_productos d ON c.producto_id = d.id_producto
    LEFT JOIN inv_categorias f ON d.categoria_id = f.id_categoria
    WHERE b.distribuidor_id = '.$distribuidor.' AND b.distribuidor_fecha >= "'.$fecha_inicial.'" AND b.distribuidor_fecha <= "'.$fecha_final.'"
