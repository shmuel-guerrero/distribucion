...
$movimientos = $db->query("SELECT o.*,cd.*, e.* 
    FROM sys_users a
	LEFT JOIN sys_empleados b ON a.persona_id = b.id_empleado
    LEFT JOIN inv_ordenes_salidas o ON b.id_empleado = o.empleado_id
    LEFT JOIN inv_ordenes_detalles cd ON cd.orden_salida_id = o.id_orden
    LEFT JOIN sys_empleados e ON o.empleado_id = e.id_empleado
	/*LEFT JOIN tmp_egresos c ON b.id_empleado = c.distribuidor_id
	LEFT JOIN gps_noventa_motivos d ON c.motivo_id = d.id_motivo
	LEFT JOIN sys_empleados e ON c.empleado_id = e.id_empleado*/
	WHERE a.rol_id = 14 AND b.id_empleado = '$id_empleado' AND o.fecha_orden >= '$fecha_inicial' AND o.fecha_orden <= '$fecha_final'")->fetch();
...
<!--                </a>-->
        ...
        <?php if ($movimientos) { ?>
            <h3 class="text-center">HISTORIAL DISTRIBUIDOR</h3>
            <div class="table-responsive">
                <table id="table" class="table table-bordered table-condensed table-restructured table-striped table-hover">
                    ...
                    <tr class="active">
                        ...
                        <th class="text-nowrap text-middle" data-datafilter-filter="true">Entrega</th>
                    ...
                    <?php foreach($movimientos as $nro => $movimiento){
                        $productos = $db->select('*, d.unidad_id as unidad_venta')
                                        ->from('inv_ordenes_salidas a')
                                        ->join('inv_ordenes_detalles d','a.id_orden=d.orden_salida_id')
                                        ->join('inv_productos b','d.producto_id=b.id_producto')
                                        ->where(array('a.id_orden'=>$movimiento['id_orden'],'id_orden'=>$movimiento['id_orden']))->fetch();
                        if($movimiento['distribuidor_estado']=='entregado'){?>
                    <tr>
                        <th class="text-nowrap"><?= $nro + 1; ?></th>
                        <td class="text-nowrap">
                            <span><?= escape($movimiento['distribuidor_fecha']); ?></span>
                            <span class="text-primary"><?= escape($movimiento['distribuidor_hora']); ?></span>
                        </td>
                        ...
                        <td class="text-nowrap text-right success"><?php foreach($productos as $producto){echo $producto['nombre_factura'].'<br>';}  ?></td>
                        ...
                        <td class="text-nowrap text-right"></td>
                        <td class="text-nowrap text-right"></td>
                        <td class="text-nowrap text-right info text-primary"><strong><?= $movimiento['nombres'].' '.$movimiento['paterno']; ?></strong></td>
                        <td class="text-nowrap text-right info"><?= $movimiento['fecha_egreso']; ?></td>
                        <td class="text-nowrap text-right info"><?= $movimiento['hora_egreso']; ?></td>
                    ...
                    <?php }else{ ?>
                            <tr>
                                <th class="text-nowrap"><?= $nro + 1; ?></th>
                                <td class="text-nowrap">
                                    <span><?= escape($movimiento['fecha_orden']); ?></span>
                                    <span class="text-primary"><?= escape($movimiento['distribuidor_hora']); ?></span>
                                </td>
                                <td class="text-nowrap"><?= (escape($movimiento['cliente'])); ?></td>
                                <td class="text-nowrap text-right text-primary"></td>
                                <td class="text-nowrap text-right"></td>
                                <td class="text-nowrap text-right"></td>
                                <td class="text-nowrap text-right"></td>
                                <td class="text-nowrap text-right danger"><?= escape($movimiento['motivo']); ?></td>
                                ...
                                <td class="text-nowrap text-right danger"><?php foreach($productos as $producto){echo $producto['nombre_factura'].'<br>';}  ?></td>
