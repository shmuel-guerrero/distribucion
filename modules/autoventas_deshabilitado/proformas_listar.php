...
$proformas = $db->select('i.*, a.almacen, a.principal, e.nombres, e.paterno, e.materno, e.cargo')
				->from('inv_ordenes_salidas i')
				->join('inv_almacenes a', 'i.almacen_id = a.id_almacen', 'left')
				->join('sys_empleados e', 'i.empleado_id = e.id_empleado', 'left')
				->where('i.fecha_orden >= ', $fecha_inicial)
				->where('i.fecha_orden <= ', $fecha_final)
				->where('i.estado>','salida')
				->where('e.id_empleado',$_user['persona_id'])
				->order_by('i.fecha_orden desc, i.hora_orden desc')->fetch();
...
<div class="panel-heading" data-formato="<?= strtoupper($formato_textual); ?>" data-mascara="<?= $formato_numeral; ?>" data-gestion="<?= date_decode($gestion_base, $_institution['formato']); ?>">
	<h3 class="panel-title">
		...
		<strong>Lista de todas las Autoventas</strong>
	</h3>
</div>
	...
	<div class="row">
		...
		<div class="col-xs-12 col-sm-4 text-right">
            ...
            <?php if ($permiso_imprimir) { ?>
                <a href="?/autoventa/proformas_imprimir" class="btn btn-primary" target="_blank" data-imprimir="true"><span class="glyphicon glyphicon-file"></span><span class="hidden-xs"> Imprimir</span></a>
            <?php } ?>
		</div>
	</div>
	<hr>
...
$(function () {
	...
	$.validate({
		...
		onSuccess: function () {
			...
			window.location = '?/autoventas/proformas_listar' + inicial_fecha + final_fecha;
		}
	});
