...
if ($id_egreso == 0) {
	...
	$valor_moneda = $moneda;
	$total = 0;
	$descuento_general=escape($egreso['descuento']);
	// Establece la fuente del contenido
	...
	foreach ($detalles as $nro => $detalle) {
		...
		$body .= '<td class="left-right">' . escape($detalle['nombre_factura'])." (".$unidad.")" . '</td>';
	...
	$descuento_monto=($total*$descuento_general/100);
	$total_descuento=$total-$descuento_monto;
	
	$valor_total_con_descuento = number_format($total_descuento, 2, '.', '');
	$descuento_monto = number_format($descuento_monto, 2, '.', '');
	...
	<table cellpadding="5">
		...
		$body
		<tr>
			<th class="all" align="left" colspan="5">MONTO TOTAL</th>
			<th class="all" align="right">$valor_total</th>
		</tr>
		<tr>
			<th class="all" align="left" colspan="5">DESCUENTO $descuento_general %</th>
			<th class="all" align="right">$descuento_monto</th>
		</tr>
		<tr>
			<th class="all" align="left" colspan="5">MONTO TOTAL CON DESCUENTO</th>
	...
	<table cellpadding="1">
		<tr>
			<td class="none">SON: $monto_escrito $valor_moneda</td>
		</tr>
	</table>
	<br><br>
	<br><br>
