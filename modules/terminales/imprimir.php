...
if ($id_terminal == 0) {
	...
	$terminales = $db->select('z.*')
	->from('inv_terminales z')
	->order_by('z.id_terminal')
	->fetch();
} else {
	...
	// Obtiene la terminal
	$terminal = $db->select('z.*')
	->from('inv_terminales z')
	->where('z.id_terminal', $id_terminal)
	->fetch_first();
...
if ($id_terminal == 0) {
	...
	$pdf->Cell(0, 10, 'LISTA DE TERMINALES', 0, true, 'C', false, '', 0, false, 'T', 'M');
	...
	// Estructura la tabla
	$body = '';
	foreach ($terminales as $nro => $terminal) {
		$body .= '<tr>';
		$body .= '<td>' . ($nro + 1) . '</td>';
		$body .= '<td>' . escape($terminal['terminal']) . '</td>';
		$body .= '<td>' . escape($terminal['impresora']) . '</td>';
		$body .= '<td>' . escape($terminal['descripcion']) . '</td>';
		$body .= '</tr>';
	}
	
	$body = ($body == '') ? '<tr><td colspan="4" align="center">No existen terminales registradas en la base de datos</td></tr>' : $body;
...
EOD;
	...
	$nombre = 'terminales_' . date('Y-m-d_H-i-s') . '.pdf';
} else {
	...
	// Titulo del documento
	$pdf->Cell(0, 10, 'DETALLE DE TERMINAL', 0, true, 'C', false, '', 0, false, 'T', 'M');
	
	// Establece la fuente del titulo
	$pdf->SetFont('helvetica', 'I', 10);
	
	// Subtitulo del documento
	$pdf->Cell(0, 5, 'ID: ' . $id_terminal, 0, true, 'C', false, '', 0, false, 'T', 'M');
	...
	$valor_terminal = escape($terminal['terminal']);
	$valor_impresora = escape($terminal['impresora']);
	$valor_descripcion = escape($terminal['descripcion']);
			...
			<td width="80%" style="border: 1px solid #444;">
				<table cellpadding="5">
					<tr>
						...
						<td width="60%">$valor_terminal</td>
					</tr>
					<tr>
						<th width="40%">Impresora:</th>
						<td width="60%">$valor_impresora</td>
					</tr>
					<tr>
						<th width="40%">Descripción:</th>
...
EOD;
	...
	// Genera el nombre del archivo
	$nombre = 'terminal_' . $id_terminal . '_' . date('Y-m-d_H-i-s') . '.pdf';
}
