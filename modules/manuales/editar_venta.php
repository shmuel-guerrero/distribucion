...
//$detalle2 = $db->select('*')->from('inv_egresos_detalles')->join('inv_productos','inv_egresos_detalles.producto_id = inv_productos.id_producto')->where('egreso_id',$id_venta)->fetch();
...
// 	LEFT JOIN inv_unidades d ON c.unidad_id = id_unidad WHERE a.egreso_id = '$id_venta' AND a.promocion_id <2 GROUP BY a.id_detalle")->fetch();
$detalles = $db->query("SELECT a.*, b.*, a.unidad_id AS unidad_det, GROUP_CONCAT(c.unidad_id, '*',d.unidad, '*', c.otro_precio SEPARATOR '|') AS prec
	FROM inv_egresos_detalles a
	LEFT JOIN inv_productos b ON a.producto_id = b.id_producto
	LEFT JOIN inv_asignaciones c ON b.id_producto = c.producto_id  AND c.visible = 's'
	LEFT JOIN inv_unidades d ON c.unidad_id = id_unidad WHERE a.egreso_id = '$id_venta' and a.promocion_id <2 AND c.visible = 's' GROUP BY a.id_detalle")->fetch();
			...
			<div class="panel-body">
				...
				<form method="post" action="?/manuales/guardar" class="form-horizontal">
					...
					<div class="form-group">
						...
						<div class="col-sm-8">
							<input type="text" value="<?= $egreso2['nit_ci'] ?>" name="nit_ci" id="nit_ci" class="form-control text-uppercase" autocomplete="off" data-validation="required number">

							<input type="hidden" value="<?= $egreso2['id_egreso'] ?>" name="egreso_id" id="egreso_id" class="form-control text-uppercase" autocomplete="off" data-validation="required number">
							<input type="hidden" value="<?= $egreso2['cliente_id'] ?>" name="id_cliente" id="id_cliente">
                            <input type="hidden" value="<?= $egreso2['credito'] ?>" name="credito" id="credito">
					...
					<div class="form-group">
						...
						<div class="col-sm-8">
							<input type="text" value="<?= $egreso2['nombre_cliente'] ?>" name="nombre_cliente" id="nombre_cliente" class="form-control text-uppercase" autocomplete="off" data-validation="required letternumber length" data-validation-allowing="-+./& " data-validation-length="max100">
						</div>
					</div>
					<div class="form-group">
						<label for="nro_factura" class="col-sm-4 control-label">Número de factura:</label>
						<div class="col-sm-8">
							<input type="text" value="<?= $egreso2['nro_factura'] ?>" name="nro_factura" id="nro_factura" class="form-control text-uppercase" autocomplete="off" data-validation="required number">
						</div>
					</div>
					<div class="form-group">
						<label for="nro_autorizacion" class="col-sm-4 control-label">Número de autorización:</label>
						<div class="col-sm-8">
							<input type="text" value="<?= $egreso2['nro_autorizacion'] ?>" name="nro_autorizacion" id="nro_autorizacion" class="form-control text-uppercase" autocomplete="off" data-validation="required number">
						</div>
					</div>
					<div class="table-responsive margin-none">
                            <table id="ventas" class="table table-bordered table-condensed table-striped table-hover table-xs margin-none">
                                ...
                                ?>
                                        ...
                                        <input type="text" value="<?= $detalle['cantidad']/cantidad_unidad($db,$detalle['producto_id'],$detalle['unidad_det']) ?>" name="cantidades[]" class="form-control input-xs text-right" maxlength="7" autocomplete="off" data-cantidad="" data-validation-allowing="range[1;<?= $stock_p2 ?>]" data-validation-error-msg="Debe ser un número positivo entre 1 y <?= $stock_p2 ?>"  data-validation="required number" onkeyup="calcular_importe(<?= $detalle['producto_id'] ?>)" > </td>
...
//                                        if($detalle['prec']){?>
                                        ...
                                        <?php } ?>
                                        <td><input type="text" value="<?= $detalle['precio'] ?>" name="precios[]" class="form-control input-xs text-right" autocomplete="off" data-precio="<?= $detalle['precio'] ?>"  data-validation-error-msg="Debe ser un número decimal positivo" onkeyup="calcular_importe(<?= $detalle['producto_id'] ?>)"></td>
					...
					<div class="form-group">
						<div class="col-xs-12">
							...
							<input type="text" name="total_importe_descuento" value="0" class="translate" tabindex="-1" data-total-descuento="" data-validation="required number" data-validation-allowing="range[0.01;1000000.00],float" data-validation-error-msg="El monto total de la venta debe ser mayor a cero y menor a 1000000.00">
						</div>
					</div>
					...
					<div id="credito_cliente">
					    <p class="text-info" id="cred">
					        <? if($egreso2['plan_de_pagos'] == 'si') {
					            echo '<div class="alert alert-info" id="borrar"> <b>Forma de pago: </b> El cliente tiene un contrato de créditos de: '.$egreso2['dias']. ' días.  </div>';
					        } else {
