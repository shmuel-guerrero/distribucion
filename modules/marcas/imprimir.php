...
if ($id_marca == 0) {
	// Obtiene las marcas
	$marcas = $db->select('z.*')->from('inv_marcas z')->order_by('z.id_marca')->fetch();
} else {
...
if ($id_marca == 0) {
	...
	$pdf->Cell(0, 10, 'LISTA DE MARCAS', 0, true, 'C', false, '', 0, false, 'T', 'M');
	...
	// Estructura la tabla
	$body = '';
	foreach ($marcas as $nro => $marca) {
		$body .= '<tr>';
		$body .= '<td>' . ($nro + 1) . '</td>';
		$body .= '<td>' . escape($marca['marca']) . '</td>';
		$body .= '<td>' . escape($marca['descripcion']) . '</td>';
		$body .= '</tr>';
	}
	
	$body = ($body == '') ? '<tr><td colspan="3" align="center">No existen marcas registradas en la base de datos</td></tr>' : $body;
	...
	<table cellpadding="5">
		<tr>
			<th width="6%">#</th>
			<th width="42.73%">Categoría</th>
			<th width="51.27%">Descripción</th>
		</tr>
		$body
	</table>
EOD;
	...
	$nombre = 'marcas_' . date('Y-m-d_H-i-s') . '.pdf';
} else {
	...
	// Titulo del documento
	$pdf->Cell(0, 10, 'DETALLE DE MARCAS', 0, true, 'C', false, '', 0, false, 'T', 'M');
	
	// Establece la fuente del titulo
	$pdf->SetFont('helvetica', 'I', 10);
	
	// Subtitulo del documento
	$pdf->Cell(0, 5, 'ID: ' . $id_marca, 0, true, 'C', false, '', 0, false, 'T', 'M');
	...
	$valor_marca = escape($marca['marca']);
	$valor_descripcion = escape($marca['descripcion']);
...
EOD;
	...
	// Genera el nombre del archivo
	$nombre = 'marca_' . $id_marca . '_' . date('Y-m-d_H-i-s') . '.pdf';
}
