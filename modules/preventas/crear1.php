								...
								<div class="col-sm-8">
									<select name="cliente" id="cliente" class="form-control text-uppercase" data-validation="letternumber" data-validation-allowing="-+./&() " data-validation-optional="true">
										...
										<?php foreach ($clientes as $cliente) { ?>
											<option value="<?= escape($cliente['nit']) . '|' . escape($cliente['cliente']) . '|' . escape($cliente['direccion']) . '|' . escape($cliente['ubicacion']) . '|' . escape($cliente['telefono']) . '|' . escape($cliente['id_cliente']). '|' . escape($cliente['cuentas_por_cobrar']); ?>"><?= escape($cliente['nit']) . ' &mdash; ' . escape($cliente['cliente']); ?></option>
										<?php } ?>
									</select>
								</div>
							</div>
								...
								<div class="col-sm-8">
									<select name="descuento_porc" id="descuento_porc" onchange="calcular_descuento_total()" class="calcular_descuento form-control" data-validation-length="max100">
										...
										<?php foreach ($porcentaje as $val) { ?>
											<option value="<?= escape($val['descuento_porcentaje']); ?>"><?= escape($val['descuento_porcentaje']); ?></option>
										<?php } ?>
									</select>
								</div>
							</div>
			...
			<div class="panel panel-default">
				...
				<div class="panel-body">
					...
					<?php if ($permiso_mostrar) : ?>
						<p class="text-right">
							<a href="?/preventas/guardar_noventa" data-toggle="tooltip" data-asignar="true" class="btn btn-danger">No vendido</a>
...
<script>
	...
	function imprimir_nota(nota) {
		window('?/preventas/imprimir_nota/' + nota, true);
		window.location.reload();
	}
