...
<div class="row">
	...
	<div class="col-md-6">
		<div class="panel panel-primary">
			...
			<div class="panel-body">
				<div class="alert alert-warning">
					...
					<p>Esta proforma tiene una validéz de 3 días, a partir de la fecha de su impresión y entrega.</p>
				</div>
				<form id="formulario" class="form-horizontal">
						...
						<table id="ventas" class="table table-bordered table-condensed table-striped table-hover table-xs margin-none">
							...
							<tbody>
								...
								<tr data-producto="<?= $detalle['producto_id']; ?>" data-detalle="<?= $detalle['producto_id']; ?>">
									...
									<td><input type="text" value="<?= $detalle_cantidad; ?>" name="cantidades[]" class="form-control input-xs text-right" maxlength="10" autocomplete="off" data-cantidad="" data-validation="required number" data-validation-allowing="range[1;10000000]" data-validation-error-msg="Debe ser un número positivo entre 1 y 0" onkeyup="calcular_importe(<?= $detalle['producto_id']; ?>)"></td>
					...
					<div class="form-group">
						<div class="col-xs-12">
							...
							<input type="text" name="nro_registros" value="<?= $proforma['nro_registros']; ?>" class="translate" tabindex="-1" data-ventas="" data-validation="required number" data-validation-allowing="range[1;25]" data-validation-error-msg="El número de productos a vender debe ser mayor a cero y menor a 25">
		...
		<div class="panel panel-primary" data-servidor="<?= ip_local . 'servidor/proforma.php'; ?>">
			<div class="panel-heading">
				<h3 class="panel-title">
	...
	<div class="col-md-6">
		<div class="panel panel-primary">
			...
			<div class="panel-body">
				...
				<div class="row">
					<div class="col-xs-12 text-right">
						<a href="?/preventas/mostrar" class="btn btn-primary"><i class="glyphicon glyphicon-list-alt"></i><span> Proformas personales</span></a>
					</div>
				</div>
				<hr>
				<?php } ?>
	...
	<div class="col-xs-12">
		<div class="panel panel-default">
			<div class="panel-heading">
				<h3 class="panel-title"><i class="glyphicon glyphicon-home"></i> Proformas de venta</h3>
			</div>
			<div class="panel-body">
				<div class="alert alert-danger">
					<strong>Advertencia!</strong>
					...
					<ul>
						<?php if (!$_terminal) { ?>
						<li>Usted no está en una terminal registrada y autorizada para la proforma de productos, debe ponerse en contacto con el administrador del sistema.</li>
						<?php } ?>
						<?php if (!$almacen) { ?>
						<li>No existe el almacén principal de la cual se puedan tomar los productos necesarios para la proforma, debe fijar un almacén principal.</li>
						<?php } ?>
					</ul>
				</div>
			</div>
...
function adicionar_producto(id_producto) {
	...
	if ($producto.size()) {
		...
		plantilla = '<tr class="active" data-producto="' + id_producto + '">' +
						...
						'<td><input type="text" value="0" name="descuentos[]" class="form-control input-xs text-right" maxlength="10" autocomplete="off" data-descuento="0" data-validation="required number" data-validation-allowing="range[-100.00;100.00],float,negative" data-validation-error-msg="Debe ser un número positivo entre -100.00 y 100.00" onkeyup="descontar_precio(' + id_producto + ')"></td>' +
	...
	$.ajax({
		...
		if (proforma) {
			$.notify({
				...
				message: '<div>La proforma fue registrada correctamente.</div>'
			}, {
				type: 'success'
			});
			...
			$.notify({
				...
				message: '<div>Ocurrió un problema al guardar los datos de la proforma, verifique si se guardó parcialmente.</div>'
			}, {
				type: 'danger'
			});
		}
