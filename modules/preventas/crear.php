...
// Obtiene el almacen
if(!isset($params[0])){
	redirect('?/preventas/seleccionar_almacen');
}
if($params[0]!='')
...
$empleados = $db->query("SELECT * FROM `sys_empleados` e
						LEFT JOIN sys_users u ON u.persona_id = e.id_empleado
						LEFT JOIN gps_rutas r ON r.empleado_id = e.id_empleado
			...
			<div class="panel-body">
				...
				<form id="formulario" class="form-horizontal">
					<div style="zoom: 1;">
                        ...
                        <div class="form-group">
                            <label for="factura_cliente" class="col-sm-4 control-label">Razon social:</label>
                            <div class="col-sm-8">
                                <input type="text" value="" name="factura_cliente" id="factura_cliente" class="form-control text-uppercase" autocomplete="off" data-validation="required letternumber length" data-validation-allowing="-+./&() " data-validation-length="max100" maxlength="100">
                            </div>
                        </div>
						<div class="form-group">
						...
						<div class="form-group">
							...
							<div class="col-sm-8">
								<input type="text" value="" name="ubicacion" id="ubicacion" class="form-control text-uppercase" autocomplete="off" data-validation="required" data-validation-allowing="-,.">
							</div>
						</div>
						...
						<div class="form-group">
							<label for="empleado" class="col-sm-4 control-label">Empleado:</label>
							<div class="col-sm-8">
								<select name="empleado" id="empleado" class="form-control text-uppercase" data-validation="required" data-validation-allowing="-+./&()" onchange="f_select_ruta(<?= $empleado['id_empleado']?>)">
								<?php if($empleados[0] != ''){?>
									<option value="">Buscar</option>
									<?php foreach ($empleados as $empleado) { ?>
										<option value="<?= escape($empleado['id_empleado']); ?>"><?= $empleado['id_empleado'].' '.escape($empleado['nombres'].' '.$empleado['paterno']); ?></option>
									<?php } ?>
								<?php }else{ ?>
									<option value="0">No existen vendedores con una ruta asignada (la venta se realizará con el usuario actual como vendedor y no se registrará la ruta)</option>
								<?php } ?>
								</select>
							</div>
						</div>
                        <div class="form-group" id="select_ruta" style="display:none">
							<label for="ruta_l" class="col-sm-4 control-label">Ruta:</label>
                            <div class="col-sm-8">
                    ...
                    <!--Descuentos-->
					<!-- ::BECA Se realizaron arreglos en descuentos -->
					<div class="col-xs-12">
					...
					<div id="credito_cliente">
					    <!--<label for="cred" class="col-md-4 control-label">Forma de Pago:</label>-->
	...
	<div class="col-md-6">
		<div class="panel panel-default">
			...
			<div class="panel-body">
				...
				<p class="text-right">                    
					<a href="?/preventas/proformas_listar" class="btn btn-success">Mis Preventas</a>
				</p>
				<?php endif ?>
				<form method="post" action="?/preventas/buscar" id="form_buscar_0" class="margin-bottom" autocomplete="off">
                ...
                <form method="post" action="?/preventas/buscar" id="form_buscar_1" class="margin-bottom" autocomplete="off">
                    <div class="form-group has-feedback">
		...
		<div class="panel panel-success">
			...
			<div class="panel-body">
				<div class="alert alert-danger">
					<p>Usted no puede realizar preventas, verifique que la siguiente información sea correcta:</p>
					<ul>
...
<!-- <h2 class="btn-success position-left-bottom display-table btn-circle margin-all display-table" data-toggle="tooltip" data-title="Esto es una preventa" data-placement="right">
                ...
                <div class="modal-body">
                    <div class="form-group">
                        ...
                        <select name="cliente" id="cliente" class="form-control text-uppercase" data-validation="letternumber" data-validation-allowing="-+./&() " data-validation-optional="true">
							<option value="">Buscar</option>
							<?php foreach ($clientes as $cliente) { ?>
								<option value="<?= escape($cliente['nit_ci']) . '|' . escape($cliente['nombre_cliente']) . '|' . escape($cliente['id_cliente']) . '|' . escape($cliente['direccion']) . '|' . escape($cliente['telefono']) . '|' . escape($cliente['ubicacion']) . '|' . escape($cliente['credito']) . '|' . escape($cliente['dias']); ?>"><?= escape($cliente['id_cliente']) . ' &mdash; ' . escape($cliente['nit_ci']) . ' &mdash; ' . escape($cliente['nombre_cliente']); ?></option>
							<?php } ?>
						</select>
                    </div>
                    <div class="form-group">
...
$(function () {
    ...
    /*
	* ::BECA
	* se borró el código que te permitía seleccionar descuento (el select fué cambiado por un input)
	*/
			...
			$.ajax({
				...
				if (productos.length) {
					...
					for (var i in productos) {
					    if ((parseInt(productos[i].cantidad_ingresos) - parseInt(productos[i].cantidad_egresos)) > 0) { // inicio if de validacion de ceros
					    ...
					    } // cerramos if de stock mayor a cero 
					}
...
function adicionar_producto(id_producto) {
	...
	if ($producto.size()) {
        ...
        if(porciones.length>2){ //arreglar pendiente
            plantilla = plantilla+'<td><select name="unidad[]" id="unidad[]" data-xxx="true" class="form-control text-lefth" >';
            ...
            plantilla = plantilla + '<td><input type="text" value="' + parte[0] + '" data-xyyz="' +stock+ '" name="unidad[]" class="form-control text-lefth" autocomplete="off" data-unidad="' + parte[0] + '" data-validation-error-msg="Debe ingresar un número decimal positivo" readonly></td>'+
...
function eliminar_producto(id_producto) {
	bootbox.confirm('&iquest;Está seguro que desea eliminar el producto?', function (result) {
		if(result){
...
function calcular_total() {
	...
	calcular_descuento_total(); 
	// let DescuentoPF=document.getElementById('DescuentoPF'),
	// 	DescuentoMF=document.getElementById('DescuentoMF'),
	// 	DescuentoTF=document.getElementById('DescuentoTF');
	// DescuentoPF.value=DescuentoPorcentaje;
	// DescuentoMF.value=DescuentoMonto;
	// DescuentoTF.value=totalA;
	// var $totald = $('[data-subtotald]:first');
	// $totald.text(totalA.toFixed(3));
	////////////////////////////////////////////////////////////////////////////--
}
...
function mostrarUbicacion(position) {
    ...
    // $('#ubicacion').val(latitud+', '+longitud);
...
/* 
* ::BECA
* la función f_select_ruta lista en el select las rutas correspondientes a ese preventista
*/
function f_select_ruta(){	
	var empleado_id = $('#empleado').val();
	
	if($("#empleado").val() < 0){
		$('#select_ruta').css({'display':'none'});		
		$('#select_ruta option')
			.removeAttr('selected')
				.filter('[value="'+id_vendedor+'"]')
					.attr('selected', true).change();
		$('#select_ruta').val(id_vendedor);
	}
	else{
		$('#select_ruta').css({'display':'block'});

		$.ajax({
			type: 'POST',
			dataType: 'json',
			url: '?/preventas/ruta_por_empleado',
			data: {
			...
			if (rutas.length > 0) {		
				...
				rutas.forEach(ruta => {
					$('#ruta')
						.append($('<option>', { value : ruta.id_ruta })
						.text(ruta.nombre));
				});				
				...
				$('#loader').fadeOut(100);
				$.notify({
					message: 'Ocurrió un problema al obtener las rutas del preventista que seleccionó'
				}, {
					type: 'danger'
				});
			}
...
function calcular_descuento_total() {
	...
	/*
	* ::BECA
	* Ésta función fue corregida en todos los crear ventas
	* Éste código ya no es necesario pues los valores siempre serán 0 (nunca habrá valores null o menor que 0) pero no será borrado por posibles consecuencias
	*/
