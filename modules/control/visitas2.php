...
// Obtiene las ventas
$proformas = $db->select('i.*, a.almacen, a.principal, e.nombres, e.paterno, e.materno')->from('inv_egresos i')->join('inv_almacenes a', 'i.almacen_id = a.id_almacen', 'left')->join('sys_empleados e', 'i.empleado_id = e.id_empleado', 'left')->join('gps_noventa_motivos no','no.id_motivo = i.motivo_id')->where('e.cargo','2')->where('i.fecha_egreso >= ', $fecha_inicial)->where('i.fecha_egreso <= ', $fecha_final)->where('i.estadoe>',0)->order_by('i.fecha_egreso desc, i.hora_egreso desc')->fetch();
...
<div class="panel-heading" data-formato="<?= strtoupper($formato_textual); ?>" data-mascara="<?= $formato_numeral; ?>" data-gestion="<?= date_decode($gestion_base, $_institution['formato']); ?>">
	<h3 class="panel-title">
		...
		<b>Lista de todas las preventas <?= $_institution['empresa2'] ?></b>
	</h3>
</div>
			...
			<table id="table" class="table table-bordered table-condensed table-striped table-hover table-xs">
				...
				<tfoot>
					<tr class="active">
                        ...
                        <th data-datafilter-filter="true">Duración</th>
						<?php if ($permiso_facturar || $permiso_ver || $permiso_eliminar || $permiso_imprimir) { ?>
				...
				<tbody>
					...
					<tr>
                        ...
                        <td><?= escape($proforma['motivo']); ?></td>
                        <td class="text-nowrap text-right"><?= escape($proforma['duracion']); ?></td>
						<?php if ($permiso_facturar || $permiso_ver || $permiso_eliminar || $permiso_imprimir) { ?>
						<td class="text-nowrap">
                            ...
                            <?php if ($permiso_imprimir) {
                                ...
                                <a href="?/control/imprimir/<?= $proforma['id_egreso']; ?>" data-toggle="tooltip" data-title="Imprimir" target="_blank" data-imprimir="true"><span class="glyphicon glyphicon-print" style="color:green"></span></a>
                            ...
                            <?php }if($proforma['estadoe']==3){ ?>
                                    <a href="?/control/imprimir/<?= $proforma['id_egreso']; ?>" data-toggle="tooltip" data-title="Imprimir" target="_blank" data-imprimir="true"><span class="glyphicon glyphicon-print" style="color:blue"></span></a>
                            <?php    }} ?>
						</td>
						<?php } ?>
					</tr>
...
$(function () {
	...
	$.validate({
		...
		onSuccess: function () {
			...
			window.location = '?/control/visitas2' + inicial_fecha + final_fecha;
		}
	});
