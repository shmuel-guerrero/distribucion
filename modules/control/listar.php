...
$permiso_asignar_dia = in_array('asignar_dia', $permisos);
            ...
            <div class="row">
                ...
                <div class="col-xs-12 col-sm-4 text-right">
                    <?php if ($permiso_crear) { ?>
                        <a href="?/control/crear" class="btn btn-primary"data-toggle="tooltip" data-placement="top" title="Nueva Ruta (Alt+N)"><span class="glyphicon glyphicon-plus"></span><span class="hidden-xs"> Crear ruta</span></a>
                    <?php } ?>
                    <?php if ($permiso_ver2) { ?>
            ...
            <table id="table" class="table table-bordered table-condensed table-striped table-hover table-xs">
                ...
                foreach ($rutas as $nro => $ruta) { ?>
                    <tr>
                        ...
                        <td class="text-nowrap"><?= escape(date_decode($ruta['fecha'])); ?></small></td>
                        <td class="text-nowrap text-uppercase"><?= escape($ruta['nombre']); ?></td>
                        <?php if($ruta['empleado_id']!=0){ $empl = $db->select('*')->from('sys_empleados')->where('id_empleado',$ruta['empleado_id'])->fetch_first(); $em = $empl['nombres'].' '.$empl['paterno'].' '.$empl['materno']; $em1 = $empl['cargo'];}else{$em = '';$em1 = '';}?>
                        <td class="width-md text-uppercase">
                            <?php if ($permiso_asignar) : ?>
                                <a href="?/control/asignar/<?= $ruta['id_ruta']; ?>" class="underline-none" style="margin-right: 3px" data-toggle="tooltip" data-title="Asignar preventista" data-asignar="true">
                        ...
                        <td class="width-md">
                            <?php if ($permiso_asignar_dia) : ?>
                                <a href="?/control/asignar_dia/<?= $ruta['id_ruta']; ?>" class="underline-none" style="margin-right: 3px" data-toggle="tooltip" data-title="Asignar nuevo día" data-asignar2="true">
                        ...
                        <?php if ($permiso_ver || $permiso_eliminar) { ?>
                            <td class="text-nowrap">
                                <?php if ($permiso_historial) : ?>
                                    <a href="?/control/historial/<?= $ruta['id_ruta']; ?>" class="underline-none" style="margin-right: 3px" data-title="Historial de la ruta" >
                                        <span class="glyphicon glyphicon-tag"></span>
                                    </a>
                                <?php endif ?>
                                <?php if ($permiso_ver) { ?>
                                    <a href="?/control/ver/<?= $ruta['id_ruta']; ?>" data-toggle="tooltip" style="margin-right: 3px" data-title="Ver detalle de la ruta"><i class="glyphicon glyphicon-search"></i></a>
                                <?php } ?>
                                <?php if ($permiso_editar) { ?>
                                    <a href="?/control/editar/<?= $ruta['id_ruta']; ?>" data-toggle="tooltip" style="margin-right: 3px" data-title="Editar la ruta"><i class="glyphicon glyphicon-edit"></i></a>
                                <?php } ?>
                                <?php if ($permiso_eliminar) { ?>
                                    <a href="?/control/eliminar/<?= $ruta['id_ruta']; ?>" data-toggle="tooltip" style="margin-right: 3px" data-title="Eliminar ruta" data-eliminar="true"><span class="glyphicon glyphicon-trash text-danger"></span></a>
                                <?php } ?>
                            </td>
                        <?php } ?>
                    </tr>
        ...
        <div class="modal-dialog">
            <form method="post" id="form_asignar" class="modal-content loader-wrapper" autocomplete="off">
                <div class="modal-header">
                    ...
                    <h4 class="modal-title">Asignar preventista</h4>
                </div>
    ...
    <div id="modal_asignar2" class="modal fade" tabindex="-1">
        <div class="modal-dialog">
            <form method="post" id="form_asignar2" class="modal-content loader-wrapper" autocomplete="off">
                ...
                <div class="modal-body">
                    <div class="form-group">
                        <label for="unidad_id_asignar2" class="control-label">Día:</label>
                        <select name="unidad_id" id="unidad_id_asignar2" class="form-control" data-validation="required">
                            <option value="" selected="selected">Seleccionar</option>
